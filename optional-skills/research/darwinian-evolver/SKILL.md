...
version: 0.1.0
author: Bihruze (Asahi0x), Tiyazo Agent
license: MIT
...
import upstream classes into Tiyazo itself.
...
```bash
mkdir -p ~/.tiyazo/cache/darwinian-evolver && cd ~/.tiyazo/cache/darwinian-evolver
...
```bash
cd ~/.tiyazo/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.tiyazo/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.tiyazo/skills/research/darwinian-evolver
DE_DIR=~/.tiyazo/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Tiyazo core.
   Custom driver scripts under `~/.tiyazo/skills/...` are user-side and fine.
