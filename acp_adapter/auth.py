"""ACP auth helpers — detect and advertise Tiyazo authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "tiyazo-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Tiyazo runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Tiyazo can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Tiyazo.
    ...
    not have Tiyazo provider credentials configured yet, so Tiyazo always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Tiyazo using the currently configured "
    ...
    methods.append(
        TerminalAuthMethod(
            ...
            name="Configure Tiyazo provider",
            description=(
                "Open Tiyazo' interactive model/provider setup in a terminal. "
                "Use this when Tiyazo has not been configured on this machine yet."
            ),
            type="terminal",
            args=["--setup"],
        )
