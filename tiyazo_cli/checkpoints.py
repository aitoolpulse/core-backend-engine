"""`tiyazo checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.tiyazo/checkpoints/``.  Actions:

    tiyazo checkpoints               # same as `status`
    tiyazo checkpoints status        # total size, project count, breakdown
    tiyazo checkpoints list          # per-project checkpoint counts + workdir
    tiyazo checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    tiyazo checkpoints clear [-f]    # nuke the entire base (asks first)
    tiyazo checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    tiyazo checkpoints
    tiyazo checkpoints prune --retention-days 3 --max-size-mb 200
    tiyazo checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: tiyazo checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``tiyazo checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `tiyazo checkpoints` → status
