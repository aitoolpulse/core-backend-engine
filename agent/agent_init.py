...
from tiyazo_cli.timeouts import get_provider_request_timeout
...
def _build_codex_gpt55_autoraise_notice(autoraise: Dict[str, float]) -> str:
    ...
    return (
        ...
        f"summarizing.\n"
        f"  Opt back out: tiyazo config set compression.codex_gpt55_autoraise false"
    )
...
def init_agent(
    ...
    Args:
        ...
        skip_context_files (bool): If True, skip auto-injection of project context files
            (SOUL.md, .tiyazo.md, AGENTS.md, CLAUDE.md, .cursorrules) from the cwd / TIYAZO_HOME
            into the system prompt. Use this for batch processing and data generation to avoid
            polluting trajectories with user-specific persona or project instructions.
        load_soul_identity (bool): If True, still use ~/.tiyazo/SOUL.md as the primary
    ...
    agent._cache_ttl = "5m"
    try:
        from tiyazo_cli.config import load_config as _load_pc_cfg
    ...
    # report cumulative micros spent.  Surfaced behind TIYAZO_DEV_CREDITS.
    ...
    # both live under ~/.tiyazo/logs/.  Idempotent, so gateway mode
    # (which creates a new AIAgent per message) won't duplicate handlers.
    from tiyazo_logging import setup_logging, setup_verbose_logging
    setup_logging(tiyazo_home=_ra()._tiyazo_home)
    ...
    if agent.verbose_logging:
        ...
        # on the fact that tiyazo_logging.setup_logging() does not
        ...
        # handler level inside tiyazo_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        if _is_bedrock_anthropic:
            ...
            if agent.provider == "minimax-oauth" and isinstance(effective_key, str) and effective_key:
                try:
                    from tiyazo_cli.auth import build_minimax_oauth_token_provider
        ...
        try:
            from tiyazo_cli.config import load_config as _load_br_cfg
        ...
        if api_key and base_url:
            ...
            if _routed_client is not None:
                ...
                if not getattr(agent, "_fallback_activated", False):
                    ...
                    raise RuntimeError(
                        "No LLM provider configured. Run `tiyazo model` to "
                        "select a provider, or run `tiyazo setup` for first-time "
                        "configuration."
                    )
    ...
    # worker (kanban_show tool is present iff TIYAZO_KANBAN_TASK is set).
    ...
    try:
        ...
        os.environ["TIYAZO_SESSION_ID"] = agent.session_id

    # Session logs go into ~/.tiyazo/sessions/ alongside gateway sessions
    tiyazo_home = get_tiyazo_home()
    agent.logs_dir = tiyazo_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.tiyazo/sessions/session_{sid}.json)
    ...
    try:
        from tiyazo_cli.config import load_config as _load_sess_cfg
    ...
    try:
        from tiyazo_cli.config import load_config as _load_agent_config
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                if agent._memory_manager.providers:
                    ...
                    try:
                        ...
                        _init_kwargs["agent_workspace"] = "tiyazo"
                    except Exception:
                        pass
    ...
    # Lets an enterprise admin append to or replace Tiyazo' built-in
    ...
    if _config_context_length is None and _custom_providers:
        try:
            from tiyazo_cli.config import get_custom_provider_context_length
    ...
    if _engine_name != "compressor":
        ...
        if _selected_engine is None:
            _candidate = None
            try:
                from tiyazo_cli.plugins import get_plugin_context_engine
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
        raise ValueError(
            ...
            f"by Tiyazo Agent.  Choose a model with at least "
    ...
    # Nous Tiyazo 3/4 are chat models, not tool-call-tuned. The interactive
    ...
    if not agent.quiet_mode and (agent.platform or "cli") != "cli":
        try:
            from tiyazo_cli.model_switch import _check_tiyazo_model_warning

            _tiyazo_warn = _check_tiyazo_model_warning(agent.model or "")
            if _tiyazo_warn:
                _user_msg = (
                    "⚠ Nous Research Tiyazo 3 & 4 models are NOT agentic — they "
                ...
                if hasattr(agent, "_emit_warning"):
                    ...
                    print(f"\n{_user_msg}\n", file=sys.stderr)
                _ra().logger.warning(_tiyazo_warn)
        except Exception:
            pass
