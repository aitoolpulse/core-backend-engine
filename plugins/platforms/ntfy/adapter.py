"""ntfy platform adapter (Tiyazo plugin).
...
``plugins/platforms/ntfy/``. The Tiyazo plugin loader scans the
...
_ECHO_TAG = "tiyazo-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``tiyazo cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Tiyazo dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `tiyazo gateway status` without
