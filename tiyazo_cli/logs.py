"""``tiyazo logs`` — view and filter Tiyazo log files.
...
Usage examples::
    ...
    tiyazo logs                    # last 50 lines of agent.log
    tiyazo logs -f                 # follow agent.log in real time
    tiyazo logs errors             # last 50 lines of errors.log
    tiyazo logs gateway -n 100    # last 100 lines of gateway.log
    tiyazo logs gui -f            # follow gui.log (dashboard/pty/ws)
    tiyazo logs desktop -f        # follow desktop.log (Electron app boot/backend)
    tiyazo logs --level WARNING    # only WARNING+ lines
    tiyazo logs --session abc123   # filter by session ID substring
    tiyazo logs --component tools  # only tool-related lines
    tiyazo logs --since 1h         # lines from the last hour
    tiyazo logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_tiyazo_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print("(Logs are created when Tiyazo runs — try 'tiyazo chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_tiyazo_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_tiyazo_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_tiyazo_home()}/logs/")
        return

    print(f"Log files in {display_tiyazo_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'tiyazo chat' to generate logs)")
