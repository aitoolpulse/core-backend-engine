# nix/tiyazo-agent.nix — Overridable Hermes Agent package
#
# callPackage auto-wires nixpkgs args; flake inputs are passed explicitly.
# Users override via:
#   pkgs.tiyazo-agent.override { extraPythonPackages = [...]; }
#   pkgs.tiyazo-agent.override { extraDependencyGroups = [ "hindsight" ]; }
{
  lib,
  stdenv,
  makeWrapper,
...
stdenv.mkDerivation (finalAttrs: {
  ...
  installPhase = ''
    ...
    mkdir -p $out/share/tiyazo-agent $out/bin
    cp -r ${bundledSkills} $out/share/tiyazo-agent/skills
    cp -r ${bundledPlugins} $out/share/tiyazo-agent/plugins
    cp -r ${bundledLocales} $out/share/tiyazo-agent/locales
    cp -r ${hermesWeb} $out/share/tiyazo-agent/web_dist

    mkdir -p $out/ui-tui
    cp -r ${hermesTui}/lib/tiyazo-tui/* $out/ui-tui/

    ${lib.concatMapStringsSep "\n"
      (name: ''
        makeWrapper ${hermesVenv}/bin/${name} $out/bin/${name} \
          --suffix PATH : "${runtimePath}" \
          --set HERMES_BUNDLED_SKILLS $out/share/tiyazo-agent/skills \
          --set HERMES_BUNDLED_PLUGINS $out/share/tiyazo-agent/plugins \
          --set HERMES_BUNDLED_LOCALES $out/share/tiyazo-agent/locales \
          --set HERMES_WEB_DIST $out/share/tiyazo-agent/web_dist \
