...
Small Node helper that bridges Tiyazo Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Tiyazo is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Tiyazo-Sidecar-Token`)
...
The Tiyazo plugin's `tiyazo photon setup` command runs `npm install`
here automatically.
