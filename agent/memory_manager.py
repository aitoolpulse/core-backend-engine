...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        _core_tool_names = set(_TIYAZO_CORE_TOOLS)
    ...
    def _strip_skill_scaffolding(text: str) -> Optional[str]:
        ...
        When a user invokes a /skill or /bundle, Tiyazo expands the turn into
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``tiyazo_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_tiyazo_home()`` themselves.
        """
        if "tiyazo_home" not in kwargs:
            from tiyazo_constants import get_tiyazo_home
            kwargs["tiyazo_home"] = str(get_tiyazo_home())
        for provider in self._providers:
            try:
