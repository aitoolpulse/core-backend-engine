...
``auth.json``.  They intentionally have no dependency on ``tiyazo_cli.auth`` so
...
# Sources Tiyazo owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Tiyazo-owned OAuth/device-code state)
