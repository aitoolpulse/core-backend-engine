#!/usr/bin/env bash
# GitHub environment detection helper for Tiyazo Agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _tiyazo_env="${TIYAZO_HOME:-$HOME/.tiyazo}/.env"; [ -f "$_tiyazo_env" ] && grep -q "^GITHUB_TOKEN=" "$_tiyazo_env" 2>/dev/null; then
