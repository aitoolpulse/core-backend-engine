...
```bash
tiyazo memory setup    # select "hindsight"
```
...
```bash
tiyazo config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.tiyazo/.env
```
...
Tiyazo spins up a local Hindsight daemon with built-in PostgreSQL. Requires an LLM API key for memory extraction and synthesis. The daemon starts automatically in the background on first use and stops after 5 minutes of inactivity.
...
Daemon startup logs: `~/.tiyazo/logs/hindsight-embed.log`
Daemon runtime logs: `~/.hindsight/profiles/<profile>.log`

To open the Hindsight web UI (local embedded mode only):
```bash
hindsight-embed -p tiyazo ui start
```
...
Config file: `~/.tiyazo/hindsight/config.json`
...
| Key | Default | Description |
|-----|---------|-------------|
| `bank_id` | `tiyazo` | Memory bank name (static fallback used when `bank_id_template` is unset or resolves empty) |
| `bank_id_template` | — | Optional template to derive the bank name dynamically. Placeholders: `{profile}`, `{workspace}`, `{platform}`, `{user}`, `{session}`. Example: `tiyazo-{profile}` isolates memory per active Tiyazo profile. Empty placeholders collapse cleanly (e.g. `tiyazo-{user}` with no user becomes `tiyazo`). |
...
| `retain_async` | `true` | Process retain asynchrotiyazoly on the Hindsight server |
| `retain_every_n_turns` | `1` | Retain every N turns (1 = every turn) |
| `retain_context` | `conversation between Tiyazo Agent and the User` | Context label for retained memories |
...
The LLM API key is stored in `~/.tiyazo/.env` as `HINDSIGHT_LLM_API_KEY`.
