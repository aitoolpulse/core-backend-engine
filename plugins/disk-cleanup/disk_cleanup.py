"""disk_cleanup — ephemeral file cleanup for Tiyazo Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under TIYAZO_HOME)
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$TIYAZO_HOME/logs/``."""
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "tiyazo-agent", "backups", "profiles", ".worktrees",
})
...
# Paths under $TIYAZO_HOME that must NEVER be deleted by quick(),
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside TIYAZO_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under TIYAZO_HOME, but never recurse into known
    # durable state trees.  Some installs place the Tiyazo checkout, venv,
