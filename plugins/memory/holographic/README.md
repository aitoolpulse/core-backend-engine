...
```bash
tiyazo memory setup    # select "holographic"
```

Or manually:
```bash
tiyazo config set memory.provider holographic
```

## Config

Config in `config.yaml` under `plugins.tiyazo-memory-store`:

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$TIYAZO_HOME/memory_store.db` | SQLite database path |
