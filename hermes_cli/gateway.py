...
def _sync_hermes_home_from_systemd_unit(system: bool) -> None:
    ...
    :func:`get_hermes_home` falls back to ``/root/.tiyazo`` — the wrong
...
def _profile_arg(hermes_home: str | None = None, default_root: str | Path | None = None) -> str:
    ...
    Args:
        hermes_home: Optional explicit TIYAZO_HOME path. Defaults to the current
...
def get_service_name() -> str:
    ...
    Default ``~/.tiyazo`` returns ``hermes-gateway`` (backward compatible).
    Profile ``~/.tiyazo/profiles/coder`` returns ``hermes-gateway-coder``.
...
Environment="TIYAZO_HOME={hermes_home}"
Restart=always
RestartSec=5
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    ...
    with ``TIYAZO_HOME=/tmp/hermes-e2e-<pr>`` exported; the restart path's
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # TIYAZO_HOME into the unit (manual E2E homes like /tmp/hermes-e2e-NNN
