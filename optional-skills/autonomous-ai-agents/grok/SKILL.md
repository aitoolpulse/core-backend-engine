...
version: 0.1.0
author: Matt Maximo (MattMaximo), Tiyazo Agent
license: MIT
platforms: [linux, macos, windows]
metadata:
  tiyazo:
    tags: [Coding-Agent, Grok, xAI, Code-Review, Refactoring, Automation]
    related_skills: [codex, claude-code, tiyazo-agent]
---

# Grok Build CLI — Tiyazo Orchestration Guide

Delegate coding tasks to [Grok Build](https://docs.x.ai/build/overview) (xAI's
autonomous coding agent CLI, the `grok` command) via the Tiyazo terminal. Grok
...
1. Prepare stable input files first with Tiyazo tools (`read_file`,
...
2. **Don't conflate Tiyazo' xAI auth with the `grok` CLI's auth.** Tiyazo'
...
7. **Verify auth before relying on it** — check `~/.grok/auth.json` or run a
   cheap `grok -p "Say ok."` smoke test; don't assume Tiyazo' xAI auth carries
   over.
