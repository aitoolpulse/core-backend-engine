...
```
Tiyazo Agent -> MCP (Streamable HTTP) -> twozero.tox (port 40404) -> TD Python
```
...
```bash
bash "${TIYAZO_HOME:-$HOME/.tiyazo}/skills/creative/touchdesigner-mcp/scripts/setup.sh"
```

The script will:
1. Check if TD is running
2. Download twozero.tox if not already cached
3. Add `twozero_td` MCP server to Tiyazo config (if missing)
...
3. **Restart Tiyazo session** to pick up the new MCP server
