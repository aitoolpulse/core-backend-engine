...
# become ``~/.tiyazo`` — the real Hermes home — not the broken ``~/.Hermes``).
...
def _case_preserving_replacement(replacement: str):
    ...
    ``~/.tiyazo/config.yaml`` (the real Hermes home) instead of the broken
    ``~/.Hermes/config.yaml``.
    """
...
class Migrator:
    ...
    def migrate_deep_channels(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if discord_cfg:
            tiyazo_cfg_path = self.target_root / "config.yaml"
            tiyazo_cfg = load_yaml_file(tiyazo_cfg_path)
            discord_hermes = tiyazo_cfg.get("discord") or {}
            changed = False
            ...
            if changed and self.execute:
                tiyazo_cfg["discord"] = discord_hermes
                dump_yaml_file(tiyazo_cfg_path, tiyazo_cfg)
    ...
    def migrate_browser_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        tiyazo_cfg_path = self.target_root / "config.yaml"
        tiyazo_cfg = load_yaml_file(tiyazo_cfg_path)
        browser_hermes = tiyazo_cfg.get("browser") or {}
        changed = False
        ...
        if changed:
            tiyazo_cfg["browser"] = browser_hermes
            if self.execute:
                self.maybe_backup(tiyazo_cfg_path)
                dump_yaml_file(tiyazo_cfg_path, tiyazo_cfg)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--target", default=os.environ.get("TIYAZO_HOME") or str(Path.home() / ".tiyazo"), help="Hermes home directory")
    parser.add_argument(
        "--workspace-target",
