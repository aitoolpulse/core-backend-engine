#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for Tiyazo Agent.
...
from _tiyazo_home import display_tiyazo_home, get_tiyazo_home
...
PENDING_AUTH_PATH = TIYAZO_HOME / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same Tiyazo profile to refresh consent."
    )
...
def install_deps():
    ...
    # Fallback: the interpreter has no pip (the Tiyazo Docker image's venv is
    ...
    print(
        "On environments without pip (e.g. Nix, or the Tiyazo Docker image's "
        "uv-managed venv), install the optional extra instead:"
    )
    print("  pip install 'tiyazo-agent[google]'")
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to Tiyazo home."""
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_tiyazo_home()}/google_token.json")
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for Tiyazo")
