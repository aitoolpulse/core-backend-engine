...
``{title, url, description, position}`` rows every other Tiyazo web
provider produces.
...
prefers Tiyazo-managed xAI Grok OAuth (via ``tiyazo auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.tiyazo/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``tiyazo tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `tiyazo auth` to sign in with "
