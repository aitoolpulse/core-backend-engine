# Tiyazo Agent Has Had "Routines" Since March
...
Tiyazo equivalent — works today:
```bash
tiyazo cron create "0 2 * * *" \
...
Tiyazo equivalent — works today:
```bash
tiyazo webhook subscribe auth-watch \
  --events "pull_request" \
...
Tiyazo equivalent — works today:
```bash
tiyazo webhook subscribe alert-triage \
...
Every use case in their blog post — backlog triage, docs drift, deploy verification, alert correlation, library porting, bespoke PR review — has a working Tiyazo implementation. No new features needed. It's been shipping since March 2026.

---

## What's Different

| | Claude Code Routines | Tiyazo Agent |
|---|---|---|
...
## Things Tiyazo Does That Routines Can't
...
Tiyazo has no daily limit. Run 500 automations a day if you want. The only constraint is your API budget, and you choose which models to use for which tasks.
...
```bash
tiyazo gateway setup    # enable webhooks
tiyazo webhook subscribe pr-review \
  --events "pull_request" \
