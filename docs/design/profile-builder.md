...
A profile = a full `~/.tiyazo/profiles/<name>/` directory with its own:
...
| Model + Provider | ModelsPage | `_write_profile_model(profile_dir, …)` | yes — TIYAZO_HOME override, already wired into create endpoint |
| MCPs | McpPage | `mcp_config._save_mcp_server` + `/api/mcp/catalog` | yes — wrap with TIYAZO_HOME override |
...
Built-in/optional skill enabling and MCP writes are **synchrotiyazo config ops**
...
1. `create_profile()` — make the dir (synchrotiyazo)
2. write model (synchrotiyazo, TIYAZO_HOME override)
3. write selected MCP servers (synchrotiyazo, TIYAZO_HOME override)
4. seed/enable selected built-in + optional skills (synchrotiyazo)
...
class ProfileCreate(BaseModel):
    ...
    mcp_servers: List[MCPServerCreate] = []      # synchrotiyazo, TIYAZO_HOME override
    builtin_skills: List[str] = []               # synchrotiyazo enable/seed
...
page afterward. Mirror `_write_profile_model`'s TIYAZO_HOME-override helper for
...
- Backend E2E with isolated TIYAZO_HOME: POST a full create body
