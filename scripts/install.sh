...
#   code at /usr/local/lib/tiyazo-agent, command at /usr/local/bin/hermes,
...
install_uv() {
    ...
    # Hermes owns its own uv at $TIYAZO_HOME/bin/uv.  Always install there —
...
check_node() {
    ...
    if [ -x "$TIYAZO_HOME/node/bin/node" ] && node_satisfies_build "$("$TIYAZO_HOME/node/bin/node" --version)"; then
        ...
        log_success "Node.js $("$TIYAZO_HOME/node/bin/node" --version) found (Hermes-managed)"
        HAS_NODE=true
        return 0
    fi
...
maybe_start_gateway() {
    ...
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            ...
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                HERMES_CMD="$(get_tiyazo_command_path)"
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            nohup $HERMES_CMD gateway > "$TIYAZO_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
