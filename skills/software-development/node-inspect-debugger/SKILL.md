...
metadata:
  tiyazo:
    ...
    related_skills: [systematic-debugging, python-debugpy, debugging-tiyazo-tui-commands]
---
...
Tiyazo-specific note: `chrome-remote-interface` is NOT in `ui-tui/package.json`. Install it to a throwaway location if you don't want to dirty the project:
...
```bash
cd /home/bb/tiyazo-agent/ui-tui
...
### Debugging a running `tiyazo --tui`
...
6. **Running `node inspect` through an agent terminal.** It's a PTY-friendly REPL. In Tiyazo, launch it with `terminal(pty=true)` or `background=true` + `process(action='submit', data='...')`. Non-PTY foreground mode will work for one-shot commands but not for interactive stepping.
