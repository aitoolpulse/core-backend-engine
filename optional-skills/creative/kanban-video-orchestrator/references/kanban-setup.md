...
the actual `setup.sh` that creates the project workspace, configures Tiyazo
...
> [Tiyazo/kanban-video-pipeline](https://github.com/Tiyazo/kanban-video-pipeline).
...
2. **Create profiles** — `tiyazo profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.tiyazo/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `tiyazo kanban create` assigned to the director
...
```bash
tiyazo profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.tiyazo/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.tiyazo/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Tiyazo
...
Each profile gets a `SOUL.md` at `~/.tiyazo/profiles/<name>/SOUL.md` that
...
```bash
tiyazo kanban create "Direct production of <video title>" \
    --assignee director \
...
the Tiyazo `.env` (`${TIYAZO_HOME:-$HOME/.tiyazo}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _tiyazo_env="${TIYAZO_HOME:-$HOME/.tiyazo}/.env"
    if grep -q "^${var}=" "$_tiyazo_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_tiyazo_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_tiyazo_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY tiyazo ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY tiyazo OPENROUTER_API_KEY || exit 1
# ...
```
