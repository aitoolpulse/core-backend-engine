...
`tiyazo skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.tiyazo.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.tiyazo.blueprint`` is a mapping containing
    ...
    blueprint = tiyazo.get("blueprint") if isinstance(tiyazo, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.tiyazo.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.tiyazo.blueprint`` block) they can hand
    to ``tiyazo skills publish`` to share. ``body`` is the plain-language
