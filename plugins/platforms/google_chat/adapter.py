...
# ``model_tools`` import time, so ``tiyazo status``, ``tiyazo chat``, etc.
...
# ``tiyazo_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Tiyazo is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a tiyazo MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${TIYAZO_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _tiyazo_home = _get_tiyazo_home()
        except (ModuleNotFoundError, ImportError):
            _tiyazo_home = _Path.home() / ".tiyazo"
        self._thread_count_store = _ThreadCountStore(
            _tiyazo_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Tiyazo is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("TIYAZO_HOME", str(_Path.home() / ".tiyazo"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${TIYAZO_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a tiyazo MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Tiyazo is thinking…' marker message.
        ...
        "Tiyazo is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Tiyazo is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Tiyazo is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Tiyazo is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Tiyazo is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``tiyazo setup``.

    The setup wizard at ``tiyazo_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.tiyazo/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. tiyazo-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.tiyazo/.env")
    print_info("Restart the gateway: tiyazo gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``tiyazo cron`` running as a
    separate process from ``tiyazo gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Tiyazo plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'tiyazo-agent[google_chat]'",
        ...
        # the "Tiyazo is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Tiyazo is thinking…' marker message "
