...
Optional Tiyazo observability plugin that maps Tiyazo observer hooks to
...
not replace Tiyazo Agent's planner, tools, memory, model provider routing, or
CLI UX. Instead, this plugin lets Tiyazo emit NeMo Relay lifecycle events for
the work Tiyazo already owns: sessions, turns, provider/API calls, tool calls,
approval prompts, and delegated subagents.

With this plugin enabled, Tiyazo Agent can:

- Preserve Tiyazo execution as NeMo Relay scopes, LLM spans, tool spans, and
  mark events.
...
```bash
env TIYAZO_HOME=/tmp/tiyazo-nemo-relay-test \
  tiyazo plugins enable observability/nemo_relay
```

Runs started with `--ignore_user_config` skip the enabled-plugin state from
`TIYAZO_HOME`, so local E2E tests should omit that flag unless the test harness
loads `observability/nemo_relay` explicitly another way.

`TIYAZO_HOME` is the Tiyazo profile/config home used by both
`tiyazo plugins enable ...` and the later `tiyazo chat ...` run. If unset,
Tiyazo uses the user's default home, usually `~/.tiyazo`. For isolated smoke
tests, choose any writable temporary directory and use the same value for every
command in that test:

```bash
export TIYAZO_HOME=/tmp/tiyazo-nemo-relay-test
tiyazo plugins enable observability/nemo_relay
tiyazo chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```

For source checkouts, make sure the `tiyazo` command you run is built from the
...
uv run tiyazo plugins enable observability/nemo_relay
uv run tiyazo chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
```bash
uv build --wheel
python -m pip install --force-reinstall dist/tiyazo_agent-*.whl
...
exposes `llm.execute(...)` / `tools.execute(...)`, Tiyazo routes LLM and tool
...
For the full generic Tiyazo middleware contract, see
...
export TIYAZO_HOME=/tmp/tiyazo-nemo-relay-docs/tiyazo-home
mkdir -p "$TIYAZO_HOME"

cat > "$TIYAZO_HOME/config.yaml" <<'YAML'
model:
  provider: custom
  default: qwen3.6:35b
...
This run starts a parent Tiyazo session, delegates to a child subagent, has the
...
{"kind":"mark","name":"tiyazo.subagent.start","metadata":{"parent_session_id":"docs-parent-session","session_id":"docs-child-session","subagent_id":"sa-0-docs","child_role":"leaf"}}
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "Tiyazo Agent E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. Tiyazo dispatches the read-only tools as one batch, and NeMo Relay
records both tool invocations.

```bash
mkdir -p /tmp/tiyazo-nemo-relay-docs/workdir
printf 'docs_parallel_alpha_function\n' > /tmp/tiyazo-nemo-relay-docs/workdir/alpha.txt
...
- Tiyazo sessions map to `agent` scopes.
- Tiyazo API request hooks map to `llm` scope start/end events.
- Tiyazo tool hooks map to `tool` scope start/end events.
...
The `observability/nemo_relay` plugin uses Tiyazo execution middleware to hand
...
When the adaptive component is enabled and the installed NeMo Relay runtime
exposes `llm.execute(...)` and `tools.execute(...)`, Tiyazo routes execution
through these boundaries:

```text
Tiyazo provider call
  -> llm_execution middleware
    -> nemo_relay.llm.execute(...)
      -> Tiyazo provider adapter next_call(...)

Tiyazo tool call
  -> tool_execution middleware
    -> nemo_relay.tools.execute(...)
      -> Tiyazo tool dispatcher next_call(...)
```
...
middleware for a local Tiyazo run. This path requires a NeMo Relay runtime that
...
```bash
export TIYAZO_HOME=/tmp/tiyazo-middleware-test/tiyazo-home
mkdir -p "$TIYAZO_HOME" /tmp/tiyazo-middleware-test/nemo-relay
...
cat > /tmp/tiyazo-middleware-test/nemo-relay/plugins.toml <<'TOML'
version = 1
...
[components.config.atof]
enabled = true
output_directory = "/tmp/tiyazo-middleware-test/atof"
filename = "middleware-events.jsonl"
mode = "overwrite"

[components.config.atif]
enabled = true
output_directory = "/tmp/tiyazo-middleware-test/atif"
filename_template = "middleware-trajectory-{session_id}.json"
agent_name = "Tiyazo Middleware E2E"
agent_version = "local"
