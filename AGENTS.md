...
**Path references in tool schemas**: If the schema description mentions file paths (e.g. default output directories), use `display_hermes_home()` to make them profile-aware. The schema is generated at import time, which is after `_apply_profile_override()` sets `TIYAZO_HOME`.

**State files**: If a tool stores persistent state (caches, logs, checkpoints), use `get_hermes_home()` for the base directory — never `Path.home() / ".tiyazo"`. This ensures each profile gets its own state.
...
`TIYAZO_HOME` before any module imports. All `get_hermes_home()` references
automatically scope to the active profile.

### Rules for profile-safe code

1. **Use `get_hermes_home()` for all TIYAZO_HOME paths.** Import from `hermes_constants`.
...
4. **Tests that mock `Path.home()` must also set `TIYAZO_HOME`** — since code now uses
   `get_hermes_home()` (reads env var), not `Path.home() / ".tiyazo"`:
   ```python
...
6. **Profile operations are HOME-anchored, not TIYAZO_HOME-anchored** — `_get_profiles_root()`
   returns `Path.home() / ".tiyazo" / "profiles"`, NOT `get_hermes_home() / "profiles"`.
...
The `_isolate_hermes_home` autouse fixture in `tests/conftest.py` redirects `TIYAZO_HOME` to a temp dir. Never hardcode `~/.tiyazo/` paths in tests.
