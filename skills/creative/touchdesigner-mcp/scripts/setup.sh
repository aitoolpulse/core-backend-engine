...
TIYAZO_HOME_DIR="${TIYAZO_HOME:-$HOME/.tiyazo}"
TIYAZO_CFG="${TIYAZO_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Tiyazo config has twozero_td MCP entry ──
if [[ ! -f "$TIYAZO_CFG" ]]; then
    echo -e " ${FAIL} Tiyazo config not found at ${TIYAZO_CFG}"
    manual_steps+=("Create ${TIYAZO_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$TIYAZO_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Tiyazo config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Tiyazo config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${TIYAZO_CFG} manually"); }
    manual_steps+=("Restart Tiyazo session to pick up config change")
fi
