#!/usr/bin/env python3
"""Live test harness for Tiyazo Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any tiyazo imports.
ORIGINAL_HOME = os.environ.get("TIYAZO_HOME")
ORIGINAL_AUTH = Path.home() / ".tiyazo" / "auth.json"
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.tiyazo/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.tiyazo/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="tiyazo_ts_live_"))
    tiyazo_home = home_dir / ".tiyazo"
    tiyazo_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, tiyazo_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".tiyazo" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, tiyazo_home / ".env")
        ...
        load_tiyazo_dotenv(tiyazo_home=str(Path.home() / ".tiyazo"))
    ...
    (tiyazo_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return tiyazo_home
...
def reset_module_state():
    """Drop cached modules so the new TIYAZO_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "tiyazo_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["TIYAZO_HOME"] = str(home)
...
def main():
    ...
    # Restore original TIYAZO_HOME
    if ORIGINAL_HOME is not None:
        os.environ["TIYAZO_HOME"] = ORIGINAL_HOME
    else:
        os.environ.pop("TIYAZO_HOME", None)
