...
The PID file lives at ``{TIYAZO_HOME}/gateway.pid``.  TIYAZO_HOME defaults to
``~/.tiyazo`` but can be overridden via the environment variable.  This means
separate TIYAZO_HOME directories naturally get separate PID files — a property
...
def _get_lock_dir() -> Path:
    ...
    override = os.getenv("TIYAZO_GATEWAY_LOCK_DIR")
    if override:
        return Path(override)
    state_home = Path(os.getenv("XDG_STATE_HOME", Path.home() / ".local" / "state"))
    return state_home / "tiyazo" / _LOCKS_DIRNAME
...
def _gateway_command_subcommand(command: str | None) -> str | None:
    """Return the Tiyazo gateway lifecycle subcommand from a command line.
    ...
    ``--profile``/``-p`` selectors from anywhere in argv -- Tiyazo's
    ...
    has_gateway_entry = (
        ...
        or any(t.rsplit("/", 1)[-1] in ("tiyazo", "tiyazo.exe") for t in tokens)
    )
    ...
    for i, token in enumerate(filtered):
        if token != "gateway":
            continue
        if i + 1 >= len(filtered):
            return "run"  # bare `tiyazo gateway` defaults to `run`
        return filtered[i + 1]
    return None
...
def looks_like_gateway_runtime_command_line(command: str | None) -> bool:
    ...
    use this broader matcher only when validating Tiyazo-owned runtime records
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the Tiyazo gateway."""
...
def _profile_name_for_home(profile_home: Path) -> Optional[str]:
    """Return the profile id a TIYAZO_HOME directory represents, or None.

    A named profile's home is ``<root>/profiles/<name>`` (immediate parent is
    ``profiles``).  The root/default home (``~/.tiyazo`` or ``$TIYAZO_HOME``)
...
def _command_line_belongs_to_profile(command: str, profile_home: Path) -> bool:
    ...
    explicit ``TIYAZO_HOME=<path>``) on its argv; the default/root gateway runs
    ...
    # a non-matching explicit TIYAZO_HOME= on the argv. TIYAZO_HOME is usually
...
def write_runtime_status(
    *,
    ...
    if served_profiles is not _UNSET:
        ...
        # for a single-profile gateway. Lets `tiyazo status` show per-profile
...
def read_runtime_status(path: Optional[Path] = None) -> Optional[dict[str, Any]]:
    ...
    can do so without mutating ``TIYAZO_HOME`` in-process.  Defaults to
...
def get_runtime_status_running_pid(
    ...
    *,
    ...
    TIYAZO_HOME.  Pass it when validating *another* profile's state file (the
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different TIYAZO_HOME dirs).
    """
...
# enabled in the user's systemd (e.g. ``tiyazo.service`` + ``tiyazo-
# gateway.service``).
#
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    # running under a different TIYAZO_HOME. When two profile gateway
    # services share the same default ~/.tiyazo (TIYAZO_HOME not set
    ...
    # field is absent in markers written by older Tiyazo versions; treat
    ...
    # ``tiyazo gateway stop`` on Windows would be misclassified as an
