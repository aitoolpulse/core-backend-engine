...
param(
    ...
    [string]$Commit = "",
    [string]$Tag = "",
    [string]$HermesHome = $(if ($env:TIYAZO_HOME) { $env:TIYAZO_HOME } else { "$env:LOCALAPPDATA\hermes" }),
    [string]$InstallDir = $(if ($env:TIYAZO_HOME) { "$env:TIYAZO_HOME\tiyazo-agent" } else { "$env:LOCALAPPDATA\hermes\tiyazo-agent" }),
...
"@ 2>$null
    ...
    if (-not $NoVenv) {
        ...
        # `hermes` command while tiyazo-agent.exe / tiyazo-acp.exe exist
...
function Set-PathVariable {
    ...
    $currentHermesHome = [Environment]::GetEnvironmentVariable("TIYAZO_HOME", "User")
    if (-not $currentHermesHome -or $currentHermesHome -ne $HermesHome) {
        [Environment]::SetEnvironmentVariable("TIYAZO_HOME", $HermesHome, "User")
        Write-Success "Set TIYAZO_HOME=$HermesHome"
    }
    $env:TIYAZO_HOME = $HermesHome
...
function Write-BootstrapMarker {
    # Writes $InstallDir\.tiyazo-bootstrap-complete which tells the Hermes
...
function Copy-ConfigTemplates {
    ...
    # Create the TIYAZO_HOME directory structure ($HermesHome, default %LOCALAPPDATA%\hermes)
...
function Write-Completion {
    ...
    Write-Host "$HermesHome\tiyazo-agent\"
    Write-Host ""
