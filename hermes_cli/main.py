...
def _apply_profile_override() -> None:
    ...
    # See issue #22502.
    hermes_home_env = os.environ.get("TIYAZO_HOME", "")
    ...
    if profile_name is not None:
        try:
            ...
            return
        os.environ["TIYAZO_HOME"] = hermes_home
...
def _ensure_tui_node() -> None:
    ...
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "TIYAZO_HOME": hermes_home},
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
...
def _hermes_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    names = set(_load_console_script_names()) or {"hermes", "tiyazo-agent", "hermes-acp"}
...
def _run_pre_update_backup(args) -> None:
    ...
    # Render path using display_hermes_home so the user sees ~/.tiyazo/...
    try:
...
def cmd_dashboard(args):
    ...
    if (
        ...
        try:
            from hermes_constants import get_default_hermes_root
            env["TIYAZO_HOME"] = str(get_default_hermes_root())
        except Exception:
