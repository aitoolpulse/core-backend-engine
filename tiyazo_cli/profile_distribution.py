"""Profile distributions — shareable, packaged Tiyazo profiles via git.

A distribution is a Tiyazo profile published as a git repository (or
...
* ``tiyazo profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``tiyazo skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``tiyazo profile``, not a parallel tree):

    tiyazo profile install <source> [--name N] [--alias] [--force] [--yes]
    tiyazo profile update  <name>  [--force-config] [--yes]
    tiyazo profile info    <name>
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "tiyazo-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            tiyazo_requires=str(data.get("tiyazo_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.tiyazo_requires:
            out["tiyazo_requires"] = self.tiyazo_requires
        if self.author:
...
def check_tiyazo_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Tiyazo {op}{target}, "
...
def _env_template_from_manifest(manifest: DistributionManifest) -> str:
    ...
    lines = [
        "# Environment variables required by this Tiyazo distribution.",
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``tiyazo profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Tiyazo profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Tiyazo distribution."
        )

    # Version check up-front so we fail fast
    check_tiyazo_requires(manifest.tiyazo_requires, tiyazo_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.tiyazo).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="tiyazo_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `tiyazo profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`tiyazo profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="tiyazo_dist_update_") as tmp:
        plan = plan_install(
