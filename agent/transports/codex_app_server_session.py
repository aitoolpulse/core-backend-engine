...
Owns one Codex thread per Tiyazo session. Drives `turn/start`, consumes
...
# Tiyazo' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_TIYAZO_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Tiyazo/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Tiyazo session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _TIYAZO_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("TIYAZO_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Tiyazo' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Tiyazo' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # auto-accept — the user already approved Tiyazo' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Tiyazo approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Tiyazo returns 'once', 'session', 'always', or 'deny'.
...
def _get_tiyazo_version() -> str:
    """Best-effort Tiyazo version string for codex's userAgent line."""
    try:
