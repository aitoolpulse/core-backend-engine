...
metadata:
  hermes:
    ...
    related_skills: [claude-code, tiyazo-agent]
---

# Codex CLI

Delegate coding tasks to [Codex](https://github.com/openai/codex) via the Tiyazo terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For Tiyazo itself, `model.provider: openai-codex` uses Tiyazo-managed Codex
OAuth from `~/.tiyazo/auth.json` after `hermes auth add openai-codex`. For the
...
## Tiyazo Gateway Caveat

When invoking the Codex CLI from a Tiyazo gateway/service context (for example,
