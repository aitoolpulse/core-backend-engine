"""Unified removal contract for every credential source Tiyazo reads from.

Tiyazo seeds its credential pool from many places:

    env:<VAR>     — os.environ / ~/.tiyazo/.env
    claude_code   — ~/.claude/.credentials.json
    tiyazo_pkce   — ~/.tiyazo/.anthropic_oauth.json
    ...
    manual        — user ran `tiyazo auth add`
...
unify here is **removal**:
    ...
    ``tiyazo auth remove <provider> <N>`` must make the pool entry stay gone.

Before this module, every source had an ad-hoc removal branch in
``auth_remove_command``, and several sources had no branch at all — so
``auth remove`` silently reverted on the next ``load_pool()`` call for
qwen-cli, nous device_code (partial), tiyazo_pkce, copilot gh_cli, and
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.tiyazo/.env  → clear it
    ...
    """
    from tiyazo_cli.config import get_env_path, remove_env_value
    ...
    if shell_exported:
        result.hints.extend([
            ...
            "launchd plist, etc.) or it will keep being visible to Tiyazo.",
            f"  The pool entry is now suppressed — Tiyazo will ignore "
            f"{env_var} until you run `tiyazo auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    work.  We just suppress it so Tiyazo stops reading it.
    """
    return RemovalResult(hints=[
        "Suppressed claude_code credential — it will not be re-seeded.",
        "Note: Claude Code credentials still live in ~/.claude/.credentials.json",
        "Run `tiyazo auth add anthropic` to re-enable if needed.",
    ])


def _remove_tiyazo_pkce(provider: str, removed) -> RemovalResult:
    """~/.tiyazo/.anthropic_oauth.json is ours — delete it outright."""
    from tiyazo_constants import get_tiyazo_home

    result = RemovalResult()
    oauth_file = get_tiyazo_home() / ".anthropic_oauth.json"
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared Tiyazo Anthropic OAuth credentials")
...
def _remove_nous_device_code(provider: str, removed) -> RemovalResult:
    ...
    `tiyazo auth add nous` (or any other path that writes providers.nous)
    from re-seeding before the user has decided to.  Suppression forces
    them to go through `tiyazo auth add nous` to re-engage, which is the
...
def _remove_xai_oauth_device_code(provider: str, removed) -> RemovalResult:
    ...
    Without this step, ``tiyazo auth remove xai-oauth <N>`` silently undoes
    ...
    if _clear_auth_store_provider(provider):
        result.cleaned.append(f"Cleared {provider} OAuth tokens from auth store")
    result.hints.append(
        "Run `tiyazo model` → xAI Grok OAuth (SuperGrok / Premium+) to re-authenticate if needed."
    )
    return result
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    the Tiyazo auth store is not enough — _seed_from_singletons() would
    ...
    via ``tiyazo auth add openai-codex``), but in both cases the re-seed
    ...
    """
    from tiyazo_cli.auth import suppress_credential_source
    ...
    result.hints.extend([
        ...
        "Run `tiyazo auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `tiyazo auth add qwen-oauth` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    Tiyazo stops picking the token up.
    """
    ...
    return RemovalResult(hints=[
        ...
        "Run `tiyazo auth add copilot` to re-enable if needed.",
    ])
...
def _register_all_sources() -> None:
    ...
    register(RemovalStep(
        provider="anthropic", source_id="tiyazo_pkce",
        remove_fn=_remove_tiyazo_pkce,
        description="~/.tiyazo/.anthropic_oauth.json",
    ))
    register(RemovalStep(
