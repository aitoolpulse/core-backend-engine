...
_API_KEY_URL = "http://app.supermemory.ai/integrations?connect=tiyazo"
...
def _load_supermemory_config(tiyazo_home: str) -> dict:
    config = _default_config()
    config_path = Path(tiyazo_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, tiyazo_home: str) -> None:
    config_path = Path(tiyazo_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class _SupermemoryClient:
    def __init__(self, api_key: str, timeout: float, container_tag: str, search_mode: str = "hybrid"):
        ...
        self._client = Supermemory(
            ...
            timeout=timeout,
            max_retries=0,
            default_headers={"x-sm-source": "tiyazo"},
        )

    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        # sm_source routes Tiyazo writes into the "Tiyazo" Space in the Supermemory
        ...
        merged = {"sm_source": "tiyazo", **(metadata or {})}
...
def _resolve_container_tag_for_setup(tiyazo_home: str, *, identity: str = "default") -> str:
    config = _load_supermemory_config(tiyazo_home)
...
def _probe_supermemory_connection(api_key: str, tiyazo_home: str, *, identity: str = "default") -> dict:
    config = _load_supermemory_config(tiyazo_home)
    status = {
        "ok": False,
        "error": "",
        "container_tag": _resolve_container_tag_for_setup(tiyazo_home, identity=identity),
        "profile_facts": 0,
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `tiyazo memory setup`.
        # All other options are documented for $TIYAZO_HOME/supermemory.json
    ...
    def save_config(self, values, tiyazo_home):
        ...
        _save_supermemory_config(sanitized, tiyazo_home)
    ...
    def get_status_config(self, provider_config: dict) -> dict:
        ...
        status = _probe_supermemory_connection(api_key, tiyazo_home)
    ...
    def post_setup(self, tiyazo_home: str, config: dict) -> None:
        ...
        from tiyazo_cli.memory_setup import _prompt, _write_env_vars
        ...
        if env_writes:
            _write_env_vars(Path(tiyazo_home) / ".env", env_writes)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        from tiyazo_constants import get_tiyazo_home
        self._tiyazo_home = kwargs.get("tiyazo_home") or str(get_tiyazo_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._tiyazo_home)
