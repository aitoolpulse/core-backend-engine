"""tiyazo hooks — inspect and manage shell-script hooks.

Usage::

    tiyazo hooks list
    tiyazo hooks test <event> [--for-tool X] [--payload-file F]
    tiyazo hooks revoke <command>
    tiyazo hooks doctor

Consent records live under ``~/.tiyazo/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.tiyazo/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``tiyazo hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: tiyazo hooks {list|test|revoke|doctor}")
        print("Run 'tiyazo hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.tiyazo/config.yaml.")
        print("See `tiyazo hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `tiyazo hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `tiyazo hooks test` and `tiyazo hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Tiyazo wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `tiyazo hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `tiyazo hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `tiyazo hooks doctor`.")
