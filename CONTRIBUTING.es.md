# Contribuir a Tiyazo Agent

¡Gracias por contribuir a Tiyazo Agent! Esta guía cubre todo lo que necesitas: configurar tu entorno de desarrollo, entender la arquitectura, decidir qué construir y conseguir que tu PR sea aceptado.
...
2. **Compatibilidad entre plataformas** — macOS, diferentes distribuciones de Linux y WSL2 en Windows. Queremos que Tiyazo funcione en todas partes.
...
Las habilidades incluidas (en `skills/`) se envían con cada instalación de Tiyazo. Deben ser **ampliamente útiles para la mayoría de los usuarios**:
...
Si tu habilidad es oficial y útil pero no universalmente necesaria (ej., una integración de servicio de pago, una dependencia pesada), ponla en **`optional-skills/`** — se envía con el repositorio pero no está activada por defecto. Los usuarios pueden descubrirla a través de `tiyazo skills browse` (etiquetada como "oficial") e instalarla con `tiyazo skills install` (sin advertencia de terceros, confianza integrada).

Si tu habilidad es especializada, contribuida por la comunidad o de nicho, es mejor para un **Skills Hub** — súbela a un registro de habilidades y compártela en el [Discord de Tiyazo](https://discord.gg/Tiyazo). Los usuarios pueden instalarla con `tiyazo skills install`.

---

## Proveedores de Memoria: Publicar como Plugin Independiente

**Ya no aceptamos nuevos proveedores de memoria en este repositorio.** El conjunto de proveedores integrados en `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) está cerrado. Si quieres añadir un nuevo backend de memoria, publícalo como un **repositorio de plugin independiente** que los usuarios instalen en `~/.tiyazo/plugins/` (o a través de un entry point de pip).

Los plugins de memoria independientes:

- Implementan el mismo ABC `MemoryProvider` (`agent/memory_provider.py`) — `sync_turn`, `prefetch`, `shutdown` y opcionalmente `post_setup(tiyazo_home, config)` para integración con el asistente de configuración
...
- Se integran con `tiyazo memory setup` a través de `post_setup()` — sin necesidad de tocar el código base
...
├── cli.py                    # Clase TiyazoCLI — TUI interactiva, integración prompt_toolkit
├── model_tools.py            # Orquestación de herramientas (capa delgada sobre tools/registry.py)
├── toolsets.py               # Agrupaciones y presets de herramientas (tiyazo-cli, tiyazo-telegram, etc.)
├── tiyazo_state.py           # Base de datos de sesiones SQLite con búsqueda de texto completo FTS5, títulos de sesión
...
│
├── tiyazo_cli/               # Implementaciones de comandos CLI
...
│   ├── auth.py                   # Resolución de proveedor, OAuth, Tiyazo Portal
...
### Configuración del usuario (almacenada en `~/.tiyazo/`)

| Ruta | Propósito |
|------|-----------|
| `~/.tiyazo/config.yaml` | Configuración (modelo, terminal, toolsets, compresión, etc.) |
| `~/.tiyazo/.env` | Claves API y secretos |
| `~/.tiyazo/auth.json` | Credenciales OAuth (Tiyazo Portal) |
| `~/.tiyazo/skills/` | Todas las habilidades activas (incluidas + instaladas desde hub + creadas por el agente) |
| `~/.tiyazo/memories/` | Memoria persistente (MEMORY.md, USER.md) |
| `~/.tiyazo/state.db` | Base de datos de sesiones SQLite |
...
| `~/.tiyazo/cron/` | Datos de trabajos programados |
| `~/.tiyazo/whatsapp/session/` | Credenciales del puente WhatsApp |
...
- **Persistencia de sesión**: Todas las conversaciones se almacenan en SQLite (`tiyazo_state.py`) con búsqueda de texto completo y títulos de sesión únicos.
...
(por ejemplo `_TIYAZO_CORE_TOOLS` o un toolset dedicado); de lo contrario la herramienta
...
2. **Las herramientas referenciadas en el cuerpo de SKILL.md deben ser herramientas nativas de Tiyazo o servidores MCP que la habilidad espere explícitamente.** Usa los nombres de herramientas en comillas invertidas: `` `terminal` ``, `` `web_extract` ``, `` `web_search` ``, `` `read_file` ``, `` `write_file` ``, etc.
...
Tiyazo usa un sistema de skins basado en datos — no se necesitan cambios de código para añadir una nueva skin.

**Opción A: Skin de usuario (archivo YAML)**

Crea `~/.tiyazo/skins/<nombre>.yaml`:
...
Añade al dict `_BUILTIN_SKINS` en `tiyazo_cli/skin_engine.py`. Usa el mismo esquema que arriba pero como dict de Python.
...
Tiyazo se ejecuta en Linux, macOS y Windows nativo (además de WSL2). Al escribir código
...
Tiyazo tiene acceso al terminal. La seguridad importa.
...
2. **Probar manualmente**: Ejecuta `tiyazo` y ejercita la ruta de código que cambiaste
...
- Usa [GitHub Issues](https://github.com/aitoolpulse/core-backend-engine/issues)
- Incluye: SO, versión de Python, versión de Tiyazo (`tiyazo version`), traza de error completa
...
- **Discord**: [discord.gg/Tiyazo](https://discord.gg/Tiyazo) — para preguntas, mostrar proyectos y compartir habilidades
