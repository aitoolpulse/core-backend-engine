...
Config (``config.yaml``)::
    ...
    hooks:
      output_spill:
        ...
        directory: null        # default: <TIYAZO_HOME>/hook_outputs
...
* Never raises. Any I/O error (disk full, permission denied, missing
  TIYAZO_HOME, etc.) falls back to a byte-length truncation with an
...
def _resolve_spill_dir(directory_override: Optional[str], session_id: Optional[str]) -> Path:
    ...
    if directory_override:
        ...
        try:
            ...
            base = Path(get_tiyazo_home()) / "hook_outputs"
        except Exception:
            # Last-resort fallback: TIYAZO_HOME env var, then ~/.tiyazo
            home = os.environ.get("TIYAZO_HOME") or os.path.expanduser("~/.tiyazo")
