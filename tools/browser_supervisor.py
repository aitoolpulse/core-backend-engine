...
One ``CDPSupervisor`` runs per Tiyazo ``task_id`` that has a reachable CDP
...
def _redact_cdp_error_text(exc: object) -> str:
    ...
    those credentials never reach Tiyazo logs or tracebacks. Falls back to a
...
DIALOG_BRIDGE_HOST = "tiyazo-dialog-bridge.invalid"
...
(() => {
  if (window.__tiyazoDialogBridgeInstalled) return;
  window.__tiyazoDialogBridgeInstalled = true;
  const ENDPOINT = "http://tiyazo-dialog-bridge.invalid/";
