...
from tiyazo_constants import get_default_tiyazo_root, get_tiyazo_home, display_tiyazo_home
...
# ``tiyazo-agent`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/tiyazo-agent/``
...
# TIYAZO_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    "tiyazo-agent",     # the codebase repo — re-clone instead
    ...
    # Python dependency trees (plugin / MCP-server venvs under TIYAZO_HOME) —
...
# Reserved archive subtree for provider state that lives OUTSIDE TIYAZO_HOME
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside TIYAZO_HOME, resolved from config only (no network, no init).
...
def _should_exclude(rel_path: Path) -> bool:
    ...
    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``tiyazo-agent`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/tiyazo-agent/`` — must be preserved.
        if part == "tiyazo-agent" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside TIYAZO_HOME.
...
def run_backup(args) -> None:
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"tiyazo-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"tiyazo-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_tiyazo_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(tiyazo_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(tiyazo_root)

        # Prune excluded directories in-place so os.walk doesn't descend
        # ``tiyazo-agent`` is only pruned at the root level; nested dirs
        # with the same name (e.g. in skills/) must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "tiyazo-agent" and not is_root)
        ]
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(tiyazo_root)
    ...
    # outside TIYAZO_HOME, so the walk above never sees it. Ask the active
    ...
    if external_to_add:
        print(
            ...
            f"stored outside {display_tiyazo_home()}."
        )
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.tiyazo/config.yaml` instead of `config.yaml`.
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_tiyazo_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (tiyazo_root / "config.yaml").exists()
        has_env = (tiyazo_root / ".env").exists()
        ...
        tiyazo_root.mkdir(parents=True, exist_ok=True)
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under TIYAZO_HOME.
            ...
            target = tiyazo_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(tiyazo_root.resolve())
            except ValueError:
        ...
        if restored_external:
            print(
                ...
                f"their original location(s) outside {display_tiyazo_home()}."
            )
        ...
        profiles_dir = tiyazo_root / "profiles"
        restored_profiles = []
...
_QUICK_STATE_FILES = (
    ...
    # are at $TIYAZO_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside TIYAZO_HOME and the entry is silently
...
def _quick_snapshot_root(tiyazo_home: Optional[Path] = None) -> Path:
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(tiyazo_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside TIYAZO_HOME. Kept in sync with
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(tiyazo_home), keep=keep)
...
def run_quick_backup(args) -> None:
    ...
    if snap_id:
        ...
        print(f"  {len(snaps)} snapshot(s) stored in {display_tiyazo_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, tiyazo_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``tiyazo_root`` to ``out_path``.
...
def _pre_update_backup_dir(tiyazo_home: Optional[Path] = None) -> Path:
    home = tiyazo_home or get_tiyazo_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of TIYAZO_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<TIYAZO_HOME>/backups/pre-update-<timestamp>.zip`` and
    ...
    result = _write_full_zip_backup(out_path, tiyazo_root)
    if result is None:
        return None
