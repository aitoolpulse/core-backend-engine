...
Tiyazo pulls API keys from Bitwarden Secrets Manager at process startup
so they don't have to live in plaintext in ``~/.tiyazo/.env``.

Design summary
--------------

* The ``bws`` binary is auto-installed into ``<tiyazo_home>/bin/bws`` on
  first use.  Tiyazo pins one version (``_BWS_VERSION``) and downloads
...
* The access token is stored in ``~/.tiyazo/.env`` as
...
* Pulling secrets is a single ``bws secret list <project_id>
  ...
  ``cache_ttl_seconds`` so back-to-back ``tiyazo`` invocations don't
  hammer the API.
* Failures NEVER block Tiyazo startup.  Missing binary, no network,
...
# Disk-persisted cache so back-to-back CLI invocations (e.g. `tiyazo chat -q ...`
...
def _disk_cache_path(home_path: Optional[Path] = None) -> Path:
    ...
    if home_path is None:
        home_path = Path(os.getenv("TIYAZO_HOME", Path.home() / ".tiyazo"))
...
def _tiyazo_bin_dir() -> Path:
    """Where Tiyazo stores its managed binaries.  Profile-aware."""
...
def install_bws(*, force: bool = False) -> Path:
    ...
    path catch these; the user-facing ``tiyazo secrets bitwarden setup``
...
def _http_download(url: str, dest: Path) -> None:
    req = urllib.request.Request(url, headers={"User-Agent": "tiyazo-agent"})
    try:
    ...
    *,
    ...
    if not access_token:
        result.error = (
            ...
            "not set.  Run `tiyazo secrets bitwarden setup`."
        )
        return result
    ...
    if not project_id:
        result.error = (
            ...
            "Run `tiyazo secrets bitwarden setup`."
        )
        return result
    ...
    if binary is None:
        result.error = (
            ...
            "Run `tiyazo secrets bitwarden setup` to install."
        )
        return result
