"""
LINE Messaging API platform adapter for Tiyazo Agent.
...
to Tiyazo Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``TIYAZO_HOME``. PR #8398.
        """
        from aiohttp import web
...
def is_connected(config) -> bool:
    """Surface in ``tiyazo status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``tiyazo status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``tiyazo setup line``.
