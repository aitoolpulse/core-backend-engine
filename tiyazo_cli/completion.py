"""Shell completion script generation for tiyazo CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_tiyazo_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Tiyazo Agent bash completion
# Add to ~/.bashrc:
#   eval "$(tiyazo completion bash)"

_tiyazo_profiles() {{
    local profiles_dir="$HOME/.tiyazo/profiles"
...
_tiyazo_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_tiyazo_profiles)" -- "$cur"))
        return
    fi
...
complete -F _tiyazo_completion tiyazo
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef tiyazo
# Tiyazo Agent zsh completion
# Add to ~/.zshrc:
#   eval "$(tiyazo completion zsh)"

_tiyazo_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.tiyazo/profiles" ]]; then
        profiles+=($HOME/.tiyazo/profiles/*(N/:t))
    fi
...
_tiyazo() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_tiyazo_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'tiyazo command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Tiyazo Agent fish completion",
        "# Add to your config:",
        "#   tiyazo completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __tiyazo_profiles",
        "    echo default",
        "    if test -d $HOME/.tiyazo/profiles",
        "        for d in $HOME/.tiyazo/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c tiyazo -f -s p -l profile"
        " -d 'Profile name' -xa '(__tiyazo_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__tiyazo_profiles)' -d 'Profile name'"
                )
