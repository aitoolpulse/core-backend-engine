...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the TIYAZO_HOME override).
...
def build_profile_secret_scope(tiyazo_home: Path) -> Dict[str, str]:
    ...
    """
    return load_env_file(Path(tiyazo_home) / ".env")
