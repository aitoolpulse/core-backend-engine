...
1. Bundled plugins: ``plugins/model-providers/<name>/`` (shipped with tiyazo-agent)
2. User plugins: ``$TIYAZO_HOME/plugins/model-providers/<name>/``
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$TIYAZO_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$TIYAZO_HOME/plugins/model-providers/`` if it exists."""
    try:
        from tiyazo_constants import get_tiyazo_home

        d = get_tiyazo_home() / "plugins" / "model-providers"
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple TIYAZO_HOME profiles don't alias each other.
    safe_name = plugin_dir.name.replace("-", "_")
    if source == "bundled":
        module_name = f"plugins.model_providers.{safe_name}"
    else:
        module_name = f"_tiyazo_user_provider_{safe_name}"
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$TIYAZO_HOME/plugins/model-providers/<name>/``
    ...
    # 1. Bundled plugins — shipped with tiyazo-agent.
    ...
    # 2. User plugins — under $TIYAZO_HOME/plugins/model-providers/<name>/.
