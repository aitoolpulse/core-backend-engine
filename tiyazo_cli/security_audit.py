"""On-demand supply-chain audit for Tiyazo Agent installs.

Scans three surfaces a Tiyazo user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The Tiyazo venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.tiyazo/plugins``
...
def _discover_plugins(tiyazo_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.tiyazo/plugins``.
    ...
    """
    plugins_dir = tiyazo_home / "plugins"
...
def run_audit(
    *,
    ...
    home = tiyazo_home or Path(get_tiyazo_home())
...
def _count_components(
    *, skip_venv: bool, skip_plugins: bool, skip_mcp: bool, tiyazo_home: Path
) -> int:
    total = 0
    if not skip_venv:
        total += len(_discover_venv())
    if not skip_plugins:
        total += len(_discover_plugins(tiyazo_home))
    if not skip_mcp:
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `tiyazo security audit`."""
    ...
    total = _count_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, tiyazo_home=home
    )
    if total == 0:
