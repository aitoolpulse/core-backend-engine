...
from tiyazo_cli.dashboard_auth import list_session_providers
from tiyazo_cli.dashboard_auth.audit import AuditEvent, audit_log
from tiyazo_cli.dashboard_auth.base import ProviderError, RefreshExpiredError
...
from tiyazo_cli.dashboard_auth.public_paths import PUBLIC_API_PATHS
...
def _auto_sso_response(request: Request) -> Response | None:
    ...
    from tiyazo_cli.dashboard_auth.cookies import detect_https
    set_sso_attempt_cookie(
...
async def gated_auth_middleware(
    ...
    # sends only ``tiyazo_session_rt``. If we bailed on ``not at`` here we'd
    ...
    if session is None:
        ...
        from tiyazo_cli.dashboard_auth.cookies import clear_session_cookies
        from tiyazo_cli.dashboard_auth.prefix import prefix_from_request
