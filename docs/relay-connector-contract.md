...
This document is the formal interface between the **Tiyazo gateway** (Python,
`gateway/relay/`) and the **connector** (Node/TypeScript,
`Tiyazo/gateway-gateway`). The connector implementer's first action is to
read this file.
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `tiyazo gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`tiyazo gateway enroll` → connector `/relay/enroll`, or
...
(`Tiyazo/gateway-gateway`); this section is the gateway-facing summary.
...
driven by the managed Portal / `tiyazo` CLI.
