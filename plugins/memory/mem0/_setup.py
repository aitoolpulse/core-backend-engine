...
def _save_mem0_json(tiyazo_home: str, data: dict) -> None:
...
def _setup_platform(tiyazo_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    provider.save_config(provider_config, tiyazo_home)
...
def _setup_oss(tiyazo_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    if not flags.get("_mode_from_flag"):
        _setup_oss_interactive(tiyazo_home, config)
        return
    ...
    user_id = flags.get("user_id") or os.getenv("USER", "tiyazo-user")
...
def _prompt_api_key(label: str, env_var: str, tiyazo_home: str) -> str:
...
def _setup_oss_interactive(tiyazo_home: str, config: dict) -> None:
    ...
    if llm_def["needs_key"]:
        key = _prompt_api_key(llm_def["label"], llm_def["env_var"], tiyazo_home)
        if key:
    ...
    if embedder_def["needs_key"] and embedder_id != llm_id:
        key = _prompt_api_key(f"{embedder_def['label']} embedder", embedder_def["env_var"], tiyazo_home)
        if key:
    ...
    if env_writes:
        _write_env(Path(tiyazo_home) / ".env", env_writes)
    _save_mem0_json(tiyazo_home, {"mode": "oss", "user_id": user_id, "agent_id": agent_id, "oss": oss_config})
...
def post_setup(tiyazo_home: str, config: dict) -> None:
    ...
    if flags["mode"] == "oss":
        ...
        _setup_oss(tiyazo_home, config, flags)
        return

    if flags["mode"] == "platform":
        _setup_platform(tiyazo_home, config, flags)
        return
