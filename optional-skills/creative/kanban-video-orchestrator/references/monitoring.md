...
tiyazo kanban watch --tenant <project-slug>

# Snapshot of the board
tiyazo kanban list --tenant <project-slug>
tiyazo kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
tiyazo kanban stats --tenant <project-slug>

# Visual dashboard (browser)
tiyazo dashboard

# Inspect a specific task (includes comments + events)
tiyazo kanban show <task-id>

# Follow a single task's event stream
tiyazo kanban tail <task-id>
```

Verify available subcommands with `tiyazo kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `tiyazo kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `tiyazo kanban block <id>` then `tiyazo kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `tiyazo kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `tiyazo kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `tiyazo profile list` (profile exists?), `tiyazo status` (gateway/dispatcher up?), and verify tenant. |
...
tiyazo kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
tiyazo kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(tiyazo kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
tiyazo kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
tiyazo kanban block <task-id>
tiyazo kanban archive <task-id>

# Diagnose what happened
tiyazo kanban show <task-id>      # task body, comments, recent events
tiyazo kanban tail <task-id>      # follow the live event stream
tiyazo kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `tiyazo kanban create` for the director
...
while true; do
    clear
    tiyazo kanban list --tenant <slug>
    echo "---"
    tiyazo kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `tiyazo kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `tiyazo profile list` and start
