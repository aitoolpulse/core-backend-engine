...
Defenses 1 and 2 (the ``_TIYAZO_GATEWAY`` guard on ``tiyazo gateway
...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.tiyazo.gateway")``,
...
State lives in ``<TIYAZO_HOME>/gateway/restart_loop.json`` so it is
...
def _state_path():
    return get_tiyazo_home() / "gateway" / "restart_loop.json"
