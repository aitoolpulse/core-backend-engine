...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("TIYAZO_HOME", os.path.expanduser("~/.tiyazo")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("TIYAZO_HOME", Path.home() / ".tiyazo")) / "skills" / "red-teaming" / "godmode"
...
TIYAZO_HOME = Path(os.getenv("TIYAZO_HOME", Path.home() / ".tiyazo"))
CONFIG_PATH = TIYAZO_HOME / "config.yaml"
PREFILL_PATH = TIYAZO_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Tiyazo models — already uncensored, just needs clean prompt
    "tiyazo": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "tiyazo" in model_lower or "nous" in model_lower:
        return "tiyazo"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Tiyazo config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.tiyazo/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Tiyazo for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Tiyazo for changes to take effect.")
