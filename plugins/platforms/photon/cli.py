"""
``tiyazo photon ...`` CLI subcommands — registered by the plugin via
...
there is no standalone ``login`` verb (matching how every other Tiyazo
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire up `tiyazo photon ...` subcommands."""
    ...
    p_setup.add_argument("--project-name", default=None,
                         help="Project name (default: 'Tiyazo Agent')")
...
def _run_device_login(args: argparse.Namespace) -> int:
    ...
    no standalone ``tiyazo photon login`` command; Photon onboards
...
def _cmd_setup(args: argparse.Namespace) -> int:
    ...
    # 2. Find or create the "Tiyazo Agent" project.
    ...
    # 3. Rotate the project secret and persist creds (runtime -> ~/.tiyazo/.env,
    ...
    print("  Start the gateway:  tiyazo gateway start")
    return 0
...
def _cmd_status(_args: argparse.Namespace) -> int:
    ...
    print(f"  sidecar deps        : {'✓ installed' if sidecar_installed else '✗ run `tiyazo photon install-sidecar`'}")
    print(f"  telemetry           : {'on' if _telemetry_enabled() else 'off'} (`tiyazo photon telemetry on|off`)")
    return 0
...
def _telemetry_enabled() -> bool:
    """Read PHOTON_TELEMETRY from the env / ~/.tiyazo/.env.
...
def _cmd_telemetry(args: argparse.Namespace) -> int:
    ...
    if state is None:
        ...
        print("  Toggle with `tiyazo photon telemetry on` / `tiyazo photon telemetry off`.")
        return 0
    try:
        ...
        return 1
    print(f"✓ Spectrum telemetry turned {state} (PHOTON_TELEMETRY in ~/.tiyazo/.env)")
    print("  Restart the gateway for the sidecar to pick it up:  tiyazo gateway restart")
    return 0
...
#
# `tiyazo gateway setup` discovers platforms via the registry and calls each
...
# project + user + sidecar flow as ``tiyazo photon setup`` with interactive
# defaults (phone is prompted when stdin is a TTY).

def gateway_setup() -> None:
    """Run Photon first-time setup from the `tiyazo gateway setup` wizard."""
