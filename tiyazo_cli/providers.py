"""
Single source of truth for provider identity in Tiyazo Agent.
...
2. **Tiyazo overlays** — transport type, auth patterns, aggregator flags,
...
# -- Tiyazo overlay ----------------------------------------------------------
# Tiyazo-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class TiyazoOverlay:
    """Tiyazo-specific provider metadata layered on top of models.dev."""
...
TIYAZO_OVERLAYS: Dict[str, TiyazoOverlay] = {
    "moa": TiyazoOverlay(
        transport="openai_chat",
        auth_type="virtual",
        base_url_override="moa://local",
    ),
    "openrouter": TiyazoOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "openai-codex": TiyazoOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "openai-api": TiyazoOverlay(
        transport="codex_responses",
        base_url_override="https://api.openai.com/v1",
        base_url_env_var="OPENAI_BASE_URL",
    ),
    "xai-oauth": TiyazoOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "qwen-oauth": TiyazoOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="TIYAZO_QWEN_BASE_URL",
    ),
    "lmstudio": TiyazoOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": TiyazoOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": TiyazoOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": TiyazoOverlay(
    ...
    "kimi-for-coding": TiyazoOverlay(
        transport="openai_chat",
        base_url_env_var="KIMI_BASE_URL",
    ),
    "stepfun": TiyazoOverlay(
        transport="openai_chat",
        extra_env_vars=("STEPFUN_API_KEY",),
        base_url_override="https://api.stepfun.ai/step_plan/v1",
        base_url_env_var="STEPFUN_BASE_URL",
    ),
    "minimax": TiyazoOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_BASE_URL",
    ),
    "minimax-oauth": TiyazoOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": TiyazoOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": TiyazoOverlay(
        transport="openai_chat",
        base_url_env_var="DEEPSEEK_BASE_URL",
    ),
    "alibaba": TiyazoOverlay(
        transport="openai_chat",
        base_url_env_var="DASHSCOPE_BASE_URL",
    ),
    "alibaba-coding-plan": TiyazoOverlay(
        transport="openai_chat",
        base_url_env_var="ALIBABA_CODING_PLAN_BASE_URL",
    ),
    "opencode": TiyazoOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": TiyazoOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "huggingface": TiyazoOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="HF_BASE_URL",
    ),
    "novita": TiyazoOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "nvidia": TiyazoOverlay(
        transport="openai_chat",
        base_url_override="https://integrate.api.nvidia.com/v1",
        base_url_env_var="NVIDIA_BASE_URL",
    ),
    "xiaomi": TiyazoOverlay(
        transport="openai_chat",
        base_url_env_var="XIAOMI_BASE_URL",
    ),
    "tencent-tokenhub": TiyazoOverlay(
        transport="openai_chat",
    ...
    "azure-foundry": TiyazoOverlay(
        transport="openai_chat",  # default; overridden by api_mode in config
        base_url_env_var="AZURE_FOUNDRY_BASE_URL",
    ),
    "bedrock": TiyazoOverlay(
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "tiyazo", "user-config"
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Tiyazo overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Tiyazo overlay
    ...
    overlay = TIYAZO_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + tiyazo extra
    ...
    if overlay is not None:
        # Tiyazo-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "") -> str:
    ...
    # Direct provider checks for providers not in TIYAZO_OVERLAYS
    if provider == "bedrock":
