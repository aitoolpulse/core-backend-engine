...
There are two ways to add a platform to the Tiyazo gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.tiyazo/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Tiyazo code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `tiyazo gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `tiyazo_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Tiyazo core.
...
And add it to the `tiyazo-gateway` composite:

```python
"tiyazo-gateway": {
    "includes": [..., "tiyazo-your-platform"]
}
```
...
## 12. Status Display (`tiyazo_cli/status.py`)
...
## 13. Gateway Setup Wizard (`tiyazo_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ tiyazo_cli/ toolsets.py \
  --include="*.py" -l | sort -u
