...
A system-level directory (default ``/etc/tiyazo``, root-owned and not
user-writable) supplies ``config.yaml`` and ``.env`` values that WIN over the
user's ``~/.tiyazo/config.yaml`` and ``~/.tiyazo/.env`` on a per-leaf-key basis.

This is DISTINCT from ``tiyazo_cli.config.is_managed()`` / ``TIYAZO_MANAGED``,
...
_DEFAULT_MANAGED_DIR = Path("/etc/tiyazo")
...
def _under_pytest() -> bool:
    ...
    Used to ignore the system default ``/etc/tiyazo`` during tests so a real
    managed scope on a developer/CI box can't leak policy into the suite. Tests
    that exercise managed scope set ``TIYAZO_MANAGED_DIR`` explicitly, which is
...
def get_managed_dir() -> Optional[Path]:
    ...
    Resolution (highest priority first):
      1. ``$TIYAZO_MANAGED_DIR`` — deployment/bootstrap path override (IT-only;
         never persisted to any .env). Honored only when set to a non-empty value
         AND the directory exists.
      2. ``/etc/tiyazo`` — POSIX default, when it exists. Ignored under pytest so
    ...
    """
    override = os.environ.get("TIYAZO_MANAGED_DIR", "").strip()
    if override:
        p = Path(override)
...
def apply_managed_overlay(config: dict) -> dict:
    ...
    (rather than going through tiyazo_cli.config.load_config) to honor managed
    scope. Mirrors tiyazo_cli.config._load_config_impl's managed merge exactly:
    ...
    try:
        ...
        from tiyazo_cli.config import _deep_merge, _expand_env_vars, _normalize_root_model_keys
