...
All per-subject data lives under PDD_DATA_DIR (default: $TIYAZO_HOME/unbroker),
which is the same trust boundary Tiyazo uses for .env and OAuth tokens.
"""
...
def tiyazo_home() -> Path:
    return Path(os.environ.get("TIYAZO_HOME") or (Path.home() / ".tiyazo"))


def data_dir() -> Path:
    override = os.environ.get("PDD_DATA_DIR")
    return Path(override) if override else tiyazo_home() / "unbroker"
