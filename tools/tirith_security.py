...
it is automatically downloaded from GitHub releases to $TIYAZO_HOME/bin/tirith.
...
def _failure_marker_path() -> str:
    ...
    return os.path.join(_get_tiyazo_home(), ".tirith-install-failed")
...
def _tiyazo_bin_dir() -> str:
    """Return $TIYAZO_HOME/bin, creating it if needed."""
    d = os.path.join(_get_tiyazo_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $TIYAZO_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_tiyazo_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $TIYAZO_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $TIYAZO_HOME/bin/tirith
    ...
    tiyazo_bin = os.path.join(_tiyazo_bin_dir(), "tirith")
    if os.path.isfile(tiyazo_bin) and os.access(tiyazo_bin, os.X_OK):
        _resolved_path = tiyazo_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return tiyazo_bin
