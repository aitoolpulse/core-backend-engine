"""`tiyazo meet node ...` subcommand tree.

Wired into the existing ``tiyazo meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``tiyazo meet node`` argparse object — typically
    ...
    run.add_argument("--display-name", default="tiyazo-meet-node")
...
def node_command(args: argparse.Namespace) -> int:
    ...
    if cmd == "run":
        ...
        print(f"             tiyazo meet node approve <name> ws://<host>:{args.port} {token}")
        try:
