# nix/nixosModules.nix — NixOS module for tiyazo-agent
#
# Two modes:
...
# container recreation. Environment variables are written to $TIYAZO_HOME/.env
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.tiyazo-agent;
    effectivePackage =
    ...
    tiyazo-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;
    ...
    # TIYAZO_HOME via addToSystemPackages, so they can save settings through the
    ...
    containerEntrypoint = pkgs.writeShellScript "tiyazo-container-entrypoint" ''
      ...
      # Ensure TIYAZO_HOME is owned by the target user.
      ...
      if [ -n "''${TIYAZO_HOME:-}" ] && [ -d "$TIYAZO_HOME" ]; then
        find "$TIYAZO_HOME" \! -user "$HERMES_UID" -exec chown "$HERMES_UID:$HERMES_GID" {} +
      fi
    ...
    # so they can update without recreation. Env vars go through $TIYAZO_HOME/.env.
  ...
  in {
    options.services.tiyazo-agent = with lib; {
      enable = mkEnableOption "Hermes Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = tiyazo-agent;
        description = "The tiyazo-agent package to use.";
      };
      ...
      stateDir = mkOption {
        ...
        description = "State directory. Contains .tiyazo/ subdir (TIYAZO_HOME).";
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $TIYAZO_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $TIYAZO_HOME/mcp-tokens/.
              '';
            };
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in tiyazo-agent's
      ...
      addToSystemPackages = mkOption {
        ...
        description = ''
          ...
          TIYAZO_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.tiyazo symlink to the service
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.tiyazo-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the hermes CLI to system PATH and export TIYAZO_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.tiyazo/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.TIYAZO_HOME = "${cfg.stateDir}/.tiyazo";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.tiyazo-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.tiyazo-agent: container.enable is true and container.hostUsers
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.tiyazo        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.tiyazo/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.tiyazo/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.tiyazo/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.tiyazo/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.tiyazo/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."tiyazo-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.tiyazo
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.tiyazo ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.tiyazo ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.tiyazo -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.tiyazo/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.tiyazo/$_subdir"
            chmod 2770 "${cfg.stateDir}/.tiyazo/$_subdir"
            find "${cfg.stateDir}/.tiyazo/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.tiyazo/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.tiyazo/config.yaml
            chmod ${configYamlMode} ${cfg.stateDir}/.tiyazo/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.tiyazo/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.tiyazo/.managed
          chmod 0644 ${cfg.stateDir}/.tiyazo/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.tiyazo/.container-mode <<'HERMES_CONTAINER_MODE_EOF'
    ...
    HERMES_CONTAINER_MODE_EOF
          ...
          '' else ''
            rm -f ${cfg.stateDir}/.tiyazo/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.tiyazo";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.tiyazo" ]; then
                  rm -f "${symlinkPath}"
                  echo "tiyazo-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.tiyazo -> stateDir/.tiyazo for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.tiyazo";
                target = "${cfg.stateDir}/.tiyazo";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "tiyazo-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.tiyazo/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.tiyazo/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.tiyazo/auth.json
              fi
            ''}
          ''}
    ...
    HERMES_NIX_ENV_EOF
        ...
        find ${cfg.stateDir}/.tiyazo/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.tiyazo/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.tiyazo/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.tiyazo-agent = {
          ...
          environment = {
            HOME = cfg.stateDir;
            TIYAZO_HOME = "${cfg.stateDir}/.tiyazo";
            HERMES_MANAGED = "true";
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              ${containerBin} create \
                ...
                --env TIYAZO_HOME=${containerDataDir}/.tiyazo \
