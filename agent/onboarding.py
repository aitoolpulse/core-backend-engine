...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Tiyazo starts and finds ``~/.openclaw/``.

    Points users at ``tiyazo claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``tiyazo claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Tiyazo, run "
        "`tiyazo claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `tiyazo claw cleanup` (renames it to ~/.openclaw.pre-migration — "
