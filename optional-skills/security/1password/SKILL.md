...
version: 1.0.0
author: arceus77-7, enhanced by Tiyazo Agent
license: MIT
...
- `tmux` available for stable authenticated sessions during Tiyazo terminal calls (desktop app flow only)
...
### Service Account (recommended for Tiyazo)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${TIYAZO_HOME:-~/.tiyazo}/.env` (the skill will prompt for this on first load).
...
## Tiyazo Execution Pattern (desktop app flow)

Tiyazo terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/tiyazo-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/tiyazo-op.sock"
