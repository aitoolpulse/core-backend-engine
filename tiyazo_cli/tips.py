...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or tiyazo -c.",
    ...
    "Ctrl+Z suspends Tiyazo to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "tiyazo -c resumes your most recent CLI session. tiyazo -c \"project name\" resumes by title.",
    "tiyazo -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "tiyazo -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "tiyazo chat -t web,terminal enables only specific toolsets for a focused session.",
    "tiyazo chat -s github-pr-workflow preloads a skill at launch.",
    "tiyazo chat -q \"query\" runs a single non-interactive query and exits.",
    "tiyazo chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "tiyazo chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "tiyazo --yolo bypasses all dangerous command approval prompts for the entire session.",
    "tiyazo chat --source telegram tags the session for filtering in tiyazo sessions list.",
    "tiyazo -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "tiyazo doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "tiyazo dump outputs a compact setup summary — great for bug reports.",
    "tiyazo config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "tiyazo config edit opens config.yaml in your default editor.",
    "tiyazo config check scans for missing or stale configuration options.",
    "tiyazo sessions browse opens an interactive session picker with search.",
    "tiyazo sessions stats shows session counts by platform and database size.",
    "tiyazo sessions prune --older-than 30 cleans up old sessions.",
    "tiyazo skills search react --source skills-sh searches the skills.sh public directory.",
    "tiyazo skills check scans installed hub skills for upstream updates.",
    "tiyazo skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "tiyazo skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "tiyazo mcp add github --command npx adds MCP servers from the command line.",
    "tiyazo mcp serve runs Tiyazo itself as an MCP server for other agents.",
    "tiyazo auth add lets you add multiple API keys for credential pool rotation.",
    "tiyazo completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "tiyazo logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "tiyazo backup creates a zip backup of your entire Tiyazo home directory.",
    "tiyazo profile create coder creates an isolated profile that becomes its own command.",
    "tiyazo profile create work --clone copies your current config and keys to a new profile.",
    "tiyazo update syncs new bundled skills to ALL profiles automatically.",
    "tiyazo gateway install sets up Tiyazo as a system service (systemd/launchd).",
    "tiyazo memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "tiyazo webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: tiyazo tools disables unused tools, tiyazo skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "tiyazo models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as tiyazo -w).",
    ...
    "TIYAZO_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Tiyazo tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'tiyazo profile create coder' creates the 'coder' command.",
    "tiyazo profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Tiyazo prints a resume command with session ID and stats.",
    "tiyazo sessions export backup.jsonl exports all sessions for backup or analysis.",
    "tiyazo -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "tiyazo skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .tiyazo/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: tiyazo cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Tiyazo runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "tiyazo gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Tiyazo via a local macOS server.",
    ...
    "Context files (.tiyazo.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "Tiyazo loads project context from .tiyazo.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "tiyazo mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "tiyazo mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "tiyazo mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — tiyazo tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.tiyazo/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "tiyazo chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "tiyazo plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "tiyazo acp runs Tiyazo as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "TIYAZO_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "tiyazo auth add nous or tiyazo auth add openai-codex sets up OAuth-based providers.",
    ...
    "tiyazo status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.tiyazo/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Tiyazo your own.",
    ...
    "Before a gateway session reset, Tiyazo auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.tiyazo/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Profiles scope Tiyazo state via TIYAZO_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "TIYAZO_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in TIYAZO_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.tiyazo/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'tiyazo profile create ls' would be rejected.",
    "tiyazo profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "tiyazo claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "TIYAZO_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via tiyazo tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.tiyazo/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.tiyazo/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Tiyazo auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Tiyazo config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Tiyazo is working.',
    ...
    '/reload reloads ~/.tiyazo/.env into the running session — pick up new API keys without restarting.',
    ...
    "TIYAZO_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.tiyazo/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.tiyazo/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'tiyazo curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "tiyazo curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'tiyazo curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'tiyazo auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    # --- TUI & Dashboard ---
    'TIYAZO_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "TIYAZO_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'tiyazo dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'tiyazo dashboard embeds the full Tiyazo TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.tiyazo/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.tiyazo/dashboard-plugins/ — no npm build required.',
    ...
    'TIYAZO_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'TIYAZO_WRITE_SAFE_ROOT restricts write_file/patch to directory prefixes; multiple paths via os.pathsep (: or ;).',
    'TIYAZO_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'TIYAZO_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.tiyazo/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.tiyazo/skills/ so drop-in skills appear without restarting the session.',
    ...
    'tiyazo -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'tiyazo chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'tiyazo chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'tiyazo chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "tiyazo chat --source tool tags programmatic chats so they don't clutter tiyazo sessions list.",
    'tiyazo dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'tiyazo sessions rename <ID> "new title" renames any past session; tiyazo sessions delete <ID> removes one.',
    'tiyazo import restores a session export or profile archive produced by sessions export or profile export.',
    'tiyazo fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'tiyazo pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'tiyazo setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'tiyazo status --deep runs the full health sweep across every component; plain tiyazo status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'TIYAZO_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'TIYAZO_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.tiyazo/plugins/ — trust-gated by design.',
    "TIYAZO_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'TIYAZO_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'TIYAZO_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'TIYAZO_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'TIYAZO_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'TIYAZO_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'TIYAZO_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'TIYAZO_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'TIYAZO_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'TIYAZO_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'TIYAZO_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Tiyazo block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from tiyazo sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.tiyazo/dashboard-plugins/.',
]
