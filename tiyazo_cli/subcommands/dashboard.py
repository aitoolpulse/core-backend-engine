"""``tiyazo dashboard`` / ``tiyazo serve`` subcommand parsers.
...
``tiyazo_cli/main.py:main()`` (god-file Phase 2); handler injected to avoid
importing ``main``.
"""
...
def _add_server_runtime_args(parser) -> None:
    ...
    # `tiyazo dashboard` / `tiyazo serve` cmdlines and SIGTERM them directly —
    # the same path `tiyazo update` uses to clean up stale servers.
    parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running Tiyazo web server processes and exit",
    )
    parser.add_argument(
        "--status",
        action="store_true",
        help="List running Tiyazo web server processes and exit",
    )
...
def build_dashboard_parser(
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Tiyazo Agent web dashboard for managing config, API keys, and sessions",
    )
    _add_server_runtime_args(dashboard_parser)
    dashboard_parser.add_argument(
        "--no-open", action="store_true", help="Don't open browser automatically"
    )
    # Backward-compat shim: older Tiyazo desktop app shells (<= 0.15.x) spawn the
    # backend as `tiyazo dashboard --no-open --tui --host ... --port ...`. The
    ...
    # showed "Tiyazo couldn't start" with no actionable cause. Accept and silently
    ...
    # browser. It exists so the Tiyazo Desktop app (and headless remote
    ...
    serve_parser = subparsers.add_parser(
        "serve",
        help="Start the Tiyazo backend server (headless; powers the desktop app and remote backends)",
        description=(
            "Run the Tiyazo backend server — the JSON-RPC/WebSocket gateway the "
    ...
    # `tiyazo dashboard register` — register a self-hosted dashboard OAuth
    # client with Nous Portal and write the client_id into ~/.tiyazo/.env.
    # Nested subparser so bare `tiyazo dashboard` keeps launching the server
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        ...
        description=(
            ...
            "TIYAZO_DASHBOARD_OAUTH_CLIENT_ID into ~/.tiyazo/.env, and prints "
            "how to engage the login gate. Requires being logged in (tiyazo setup)."
        ),
    )
    ...
    dashboard_register_parser.add_argument(
        ...
        help=(
            ...
            "https://tiyazo.example.com/auth/callback. Omit for localhost-only use."
        ),
    )
    dashboard_register_parser.add_argument(
        "--portal-url",
        ...
        help=(
            ...
            "portal. Also settable via TIYAZO_DASHBOARD_PORTAL_URL. Mainly for "
