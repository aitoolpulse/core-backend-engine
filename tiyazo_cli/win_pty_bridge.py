"""Windows ConPTY bridge for the `tiyazo dashboard` chat tab.

Drop-in counterpart to ``tiyazo_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``tiyazo_cli.web_server`` consumes: ``spawn``, ``read``,
