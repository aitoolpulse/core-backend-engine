...
- Supermemory API key from [app.supermemory.ai/integrations?connect=tiyazo](http://app.supermemory.ai/integrations?connect=tiyazo)

## Setup

```bash
tiyazo memory setup    # select "supermemory"
```

Or manually:

```bash
tiyazo config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.tiyazo/.env
```

## Config

Config file: `$TIYAZO_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `tiyazo` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `tiyazo-{identity}` → `tiyazo-coder`). |
...
All Supermemory API calls send `x-sm-source: tiyazo`, and document writes stamp
`metadata.sm_source: tiyazo`. This is a **functional routing key, not telemetry**:
it groups Tiyazo-written memories into a dedicated "Tiyazo" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Tiyazo profile:

```json
{
  "container_tag": "tiyazo-{identity}"
}
```

For a profile named `coder`, this resolves to `tiyazo-coder`. The default profile resolves to `tiyazo-default`. Without `{identity}`, all profiles share the same container.
