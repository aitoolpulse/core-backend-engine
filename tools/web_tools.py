...
Backend is selected during ``tiyazo tools`` setup (web.backend in config.yaml).
When available, Tiyazo can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _env_value(name: str) -> str:
    """Resolve ``name`` via Tiyazo config-aware env, falling back to process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set
    through Tiyazo' config/.env layer (``tiyazo config set``, ``tiyazo tools``)
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``tiyazo tools``).
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `tiyazo tools` repaint.
        try:
        ...
        if provider is None:
            response_data = {
                ...
                "error": (
                    ...
                    "Run `tiyazo tools` to set one up."
                ),
            }
        else:
            logger.info(
