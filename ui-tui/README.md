...
React + Ink terminal UI for Tiyazo. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `TIYAZO_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `tiyazo setup`, suspends Ink while it runs, opens a new session on success
...
- `commands/billing.ts` — `/billing`: manage Tiyazo terminal billing — buy credits, auto-reload, limits
...
- Input history is stored in `~/.tiyazo/.tiyazo_history` or under `TIYAZO_HOME`.
...
### Billing (`billing.ts`)
`/billing` — manage Tiyazo terminal billing — buy credits, auto-reload, limits
...
### Credits (`credits.ts`)
`/credits` — Tiyazo credit balance and browser top-up

### Setup (`setup.ts`)
`/setup` — launches external `tiyazo setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/tiyazo-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external tiyazo setup, suspends Ink while it runs
      ...
      slash/
        ...
        commands/
          billing.ts                /billing — manage Tiyazo terminal billing
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.tiyazo/tui-parent.log
    ...
    types/
      tiyazo-ink.d.ts            type declarations for @tiyazo/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent TiyazoCLI subprocess for slash commands
```