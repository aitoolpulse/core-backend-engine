...
Profile-aware location: ``$TIYAZO_HOME/logs/dashboard-auth.log``.
...
from ``tiyazo_constants`` or other tiyazo_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$TIYAZO_HOME/logs/dashboard-auth.log`` with the standard fallback.

    Mirrors ``tiyazo_constants.get_tiyazo_home`` semantics: env var wins,
    else ``~/.tiyazo``. A local copy avoids an import cycle with the
    middleware which lives below ``tiyazo_cli``.
    """
    home = os.environ.get("TIYAZO_HOME") or str(Path.home() / ".tiyazo")
