...
transport, discovers their tools, and registers them into the tiyazo-agent
tool registry so the agent can call them like any built-in tool.

Configuration is read from ~/.tiyazo/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.tiyazo/logs/mcp-stderr.log), tagged with the
...
def _check_logging_callback_support() -> bool:
    ...
    server emits, so server-side diagnostics never reach Tiyazo' logs.
    """
    if not _MCP_AVAILABLE:
        return False
    try:
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            tiyazo_home = os.path.expanduser(
                os.getenv(
                    "TIYAZO_HOME", os.path.join(os.path.expanduser("~"), ".tiyazo")
                )
            )
            candidates = [
                os.path.join(tiyazo_home, "node", "bin", resolved_command),
                ...
                # image (which the Tiyazo Docker image copies node + npm +
...
# ---------------------------------------------------------------------------
# MCP ImageContent block → Tiyazo MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that Tiyazo gateways know how to render.
...
class ElicitationHandler:
    ...
    Form-mode elicitations are routed through Tiyazo' existing approval
    ...
    async def __call__(self, context, params):
        ...
        # the agent's contextvars (TIYAZO_SESSION_PLATFORM etc.). When
...
class MCPServerTask:
    ...
    def __init__(self, name: str):
        ...
        # context doesn't inherit TIYAZO_SESSION_PLATFORM, so the
    ...
    def _make_logging_callback(self):
        ...
        server into Tiyazo' logging (agent.log via tiyazo_logging), tagged
    ...
    async def _run_stdio(self, config: dict):
        ...
        if not _MCP_AVAILABLE:
            raise ImportError(
                ...
                "  pip install 'tiyazo-agent[mcp]'\n"
                "or (full install):\n"
                "  pip install 'tiyazo-agent[all]'"
            )
    ...
    async def run(self, config: dict):
        ...
        # routes those requests through Tiyazo' approval system.
        ...
        while True:
            try:
                ...
                # and the MCP server would stay dead until Tiyazo is fully
...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"Run `tiyazo mcp login {server_name}` (or delete the tokens "
            f"file under ~/.tiyazo/mcp-tokens/ and restart). Do NOT retry "
...
def _wrap_with_home_override(coro: "Coroutine") -> "Coroutine":
    """Carry the caller's context-local TIYAZO_HOME override into ``coro``.
    ...
    try:
        ...
        home_override = get_tiyazo_home_override()
    except Exception:
        return coro
    ...
    async def _scoped():
        token = set_tiyazo_home_override(home_override)
        try:
            return await coro
        finally:
...
def _run_on_mcp_loop(coro_or_factory, timeout: float = 30):
    ...
    # Propagate the context-local TIYAZO_HOME override onto the MCP loop.
    ...
    # vanish here: OAuth token stores and any other get_tiyazo_home()
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the Tiyazo config file.
    ...
    ``os.environ`` (which includes ``~/.tiyazo/.env`` loaded at startup).
    """
    try:
        ...
        if _env_enabled("TIYAZO_SAFE_MODE"):
            return {}
        config = load_config()
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            # flow through Tiyazo' MEDIA: tag convention and out to messaging
            ...
            # Tiyazo' MEDIA tag + cache_image_from_bytes) was the cleaner of
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves Tiyazo's historical behavior of converting hyphens to
...
# Native MCP tool-name prefix. Tiyazo uses the ``mcp__<server>__<tool>``
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the Tiyazo registry schema format.
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``tiyazo tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the Tiyazo registry.
...
def _kill_orphaned_mcp_children(include_active: bool = False) -> None:
    ...
    survivors, avoiding shared-resource collisions when multiple tiyazo
