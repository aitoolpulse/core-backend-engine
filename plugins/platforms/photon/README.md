...
This plugin connects Tiyazo Agent to iMessage (and other Spectrum
...
TypeScript-only, Tiyazo runs it inside a small supervised Node sidecar and
...
There is no separate `login` command; like every other Tiyazo channel,
...
Run `tiyazo photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.tiyazo/.env` (the same place every other
...
Management metadata lives in `~/.tiyazo/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `tiyazo photon telemetry on\|off` (restart the gateway to apply) |
...
6. Verify end-to-end: `tiyazo photon status`, a DM and a group roundtrip,
