...
class PlatformEntry:
    ...
    # Env vars this platform needs (for ``tiyazo setup`` display).
    ...
    # built-ins).  Used by ``tiyazo gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # plugin-discovery time added several seconds to *every* `tiyazo`
        # invocation -- including plain `tiyazo chat`, which never touches any
        ...
        # `tiyazo setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``tiyazo setup``/``gateway status``, channel
