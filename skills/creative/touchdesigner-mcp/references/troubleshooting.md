...
## 5. Tiyazo Configuration

### Config location

`$TIYAZO_HOME/config.yaml` (defaults to `~/.tiyazo/config.yaml` when `TIYAZO_HOME` is unset)
...
Restart the Tiyazo session for changes to take effect. The MCP connection is
