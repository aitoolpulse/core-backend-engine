"""
Lazy dependency installer for opt-in Tiyazo Agent backends.

Many Tiyazo features (Mistral TTS, ElevenLabs TTS, Honcho memory, Bedrock,
...
(``tiyazo-agent[all]``) and install them eagerly at setup time. That has
two problems:
...
remediation hint pointing at ``tiyazo tools`` or the manual pip command.
...
* **Durable-target mode (immutable images).** When the deployment seals the
  agent's own venv (the Docker image sets ``TIYAZO_DISABLE_LAZY_INSTALLS=1``
  and makes ``/opt/tiyazo`` read-only), setting
  ``TIYAZO_LAZY_INSTALL_TARGET`` redirects lazy installs to a writable
  ...
  installed package cannot brick Tiyazo, which is what made it safe to seal
...
LAZY_DEPS: dict[str, tuple[str, ...]] = {
    ...
    # (TIYAZO_DISABLE_LAZY_INSTALLS=1) and lazy installs are redirected to the
...
_LAZY_TARGET_ENV = "TIYAZO_LAZY_INSTALL_TARGET"
...
def _allow_lazy_installs() -> bool:
    ...
    2. ``TIYAZO_DISABLE_LAZY_INSTALLS=1`` seals the *agent venv* (set by the
    ...
    if os.environ.get("TIYAZO_DISABLE_LAZY_INSTALLS") == "1":
...
def _unsupported_feature_reason(feature: str) -> Optional[str]:
    ...
    ``tiyazo update`` lazy-refresh pass.
    """
    if sys.platform == "win32" and feature == "platform.matrix":
        return (
            ...
            "from sdist. Run Tiyazo under WSL to use Matrix on Windows."
        )
    return None
...
def _is_satisfied(spec: str) -> bool:
    ...
    ``tiyazo update`` propagate pin bumps in :data:`LAZY_DEPS` to already-
...
def active_features() -> list[str]:
    ...
    Used by ``tiyazo update`` to figure out which lazy backends need a
...
def refresh_active_features(*, prompt: bool = False) -> dict[str, str]:
    ...
    Intended for ``tiyazo update``. Never raises; lazy-install failures
