...
Let the tiyazo agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where tiyazo runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `tiyazo meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `tiyazo meet` CLI |
...
| `cli.py` | `tiyazo meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$TIYAZO_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `tiyazo meet node {run,list,approve,remove,status,ping}` |
...
tiyazo meet install                                      # pip + Chromium
tiyazo meet setup                                        # preflight
tiyazo meet auth                                         # optional
tiyazo meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
tiyazo meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.tiyazo/.env
tiyazo meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
tiyazo meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
tiyazo meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.tiyazo/.env
tiyazo meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, tiyazo will **not** switch your system audio input automatically — the
...
tiyazo plugins enable google_meet
tiyazo meet node run --display-name my-mac --host 0.0.0.0 --port 18789
