#!/usr/bin/env python3
"""Canvas LMS API CLI for Tiyazo Agent.
...
def _check_config():
    ...
    if missing:
        tiyazo_env = os.path.join(
            os.environ.get("TIYAZO_HOME", os.path.expanduser("~/.tiyazo")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {tiyazo_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Tiyazo Agent"
    )
