...
~/.tiyazo/skills/. Existing skills (bundled, hub-installed, or user-created)
...
def _guard_agent_created_enabled() -> bool:
    ...
    on via `tiyazo config set skills.guard_agent_created true`.
    """
    try:
    ...
    try:
        ...
        if rec.get("pinned"):
            return (
                ...
                f"`tiyazo curator unpin {name}` if they want to delete it. "
        ...
        if skill_usage.get_record(name).get("pinned"):
            return {
                ...
                "error": (
                    ...
                    f"`tiyazo curator unpin {name}` if they want it changed."
                ),
            }
    except Exception:
...
def _find_skill(name: str) -> Optional[Dict[str, Any]]:
    ...
    Searches the local skills dir (~/.tiyazo/skills/) first, then any
...
def _find_skill_in_other_profiles(name: str) -> List[Tuple[str, Path]]:
    """Look for ``name`` under SKILL.md across OTHER Tiyazo profiles.
    ...
    # Default profile (~/.tiyazo/skills) — only consider when active is non-default.
    ...
    # All named profiles (~/.tiyazo/profiles/*/skills)
    ...
    if others:
        if len(others) == 1:
            ...
            base += (
                ...
                f"another profile, switch profiles (`tiyazo -p "
            ...
            base += (
                ...
                f"Switch profiles (`tiyazo -p <name>`) to edit there, or "
...
def _delete_skill(name: str, absorbed_into: Optional[str] = None) -> Dict[str, Any]:
    ...
    # RECOVERABLE: archival into ~/.tiyazo/skills/.archive/ is documented as
    # the maximum destructive action the curator may take, and
    # `tiyazo curator restore` promises the skill can be brought back. Route
    ...
    if result.get("success"):
        ...
        try:
            ...
            if action == "create":
                ...
                # keeps its usage record as STATE_ARCHIVED so `tiyazo curator
...
SKILL_MANAGE_SCHEMA = {
    ...
    "description": (
        ...
        "will refuse with a message pointing the user to `tiyazo curator unpin <name>`. "
