...
Every Tiyazo request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to Tiyazo Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=tiyazo-agent",
        "client=tiyazo-client-v<__version__>",
    ]

The version is sourced live from ``tiyazo_cli.__version__`` so it auto-aligns
...
``tiyazo_cli.__version__`` is the canonical source of truth.
"""
...
def _tiyazo_version() -> str:
    """Return the current Tiyazo release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``tiyazo_cli`` cannot be imported (should
...
def tiyazo_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=tiyazo-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=tiyazo-client-v{_tiyazo_version()}"
...
def nous_portal_tags() -> List[str]:
    ...
    """
    return ["product=tiyazo-agent", tiyazo_client_tag()]
