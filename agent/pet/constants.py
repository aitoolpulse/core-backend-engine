...
row taxonomy is inferred from the atlas shape so Tiyazo can render both legacy
...
# rows (1536x1872), while older Tiyazo/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Tiyazo' activity state names. They are not always identical to the
...
# Legacy Tiyazo/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Tiyazo activity names -> accepted row-name aliases in descending
