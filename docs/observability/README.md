# Tiyazo Observer Hooks

Tiyazo observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Tiyazo' planner, model providers, memory, tool registry,
...
```text
telemetry_schema_version = "tiyazo.observer.v1"
```

Hook callbacks are fail-open. Tiyazo catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Tiyazo only builds
