"""``tiyazo debug`` subcommand parser.
...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Tiyazo Agent. Use 'tiyazo debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    tiyazo debug share              Upload debug report (asks for confirmation)
    tiyazo debug share --yes        Skip confirmation (for scripts/CI)
    tiyazo debug share --lines 500  Include more log lines
    tiyazo debug share --expire 30  Keep paste for 30 days
    tiyazo debug share --local      Print report locally (no upload)
    tiyazo debug share --no-redact  Disable upload-time secret redaction
    tiyazo debug share --nous       Upload to Nous-internal storage (private)
    tiyazo debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'tiyazo debug share'",
    )
