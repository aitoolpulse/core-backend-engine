...
import tiyazo_cli.auth as auth_mod
...
def _write_through_provider_state_to_global_root(
    ...
    ``tiyazo_cli.auth._write_through_xai_oauth_to_global_root`` (which covers
...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``tiyazo model`` / ``tiyazo auth`` which
    ...
    def _sync_xai_oauth_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        xAI OAuth refresh tokens are single-use.  When another Tiyazo process
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        ``tiyazo`` invocation that defaults to the active provider
        (e.g. setup wizard, ``tiyazo auth status``) would land on
        ...
        try:
            with _auth_store_lock():
                ...
                # tiyazo_cli.auth._save_xai_oauth_tokens (#43589); the pool
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        # sequence below must run atomically across Tiyazo processes: otherwise
        ...
        if self.provider == "openai-codex":
            refresh_timeout_seconds = auth_mod.env_float(
                "TIYAZO_CODEX_REFRESH_TIMEOUT_SECONDS", 20
            )
            lock_timeout = max(
        ...
        try:
            if self.provider == "anthropic":
                ...
                refreshed = refresh_anthropic_oauth_pure(
                    entry.refresh_token,
                    use_json=entry.source.endswith("tiyazo_pkce"),
                )
                updated = replace(
                    entry,
                ...
                # refresh_token — single-use tokens consumed by another Tiyazo
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        ...
                        refreshed = refresh_anthropic_oauth_pure(
                            ...
                            use_json=synced.source.endswith("tiyazo_pkce"),
                        )
                        updated = replace(
                            synced,
                            access_token=refreshed["access_token"],
            ...
            if self.provider == "xai-oauth":
                ...
                if auth_mod._is_terminal_xai_oauth_refresh_error(exc):
                    ...
                    return None
            # For openai-codex: same race as xAI/nous — another Tiyazo process
    ...
    def _available_entries(self, *, clear_expired: bool = False, refresh: bool = False) -> List[PooledCredential]:
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other Tiyazo profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
            ...
            # re-authed via `tiyazo model` / `tiyazo auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `tiyazo model` ->
            ...
            if entry.last_status == STATUS_DEAD:
                ...
                # user can always re-add via ``tiyazo auth add``.  Singleton-
                ...
                if _is_manual_source(entry.source):
                    ...
                    if dead_at and now - dead_at > DEAD_MANUAL_PRUNE_TTL_SECONDS:
                        ...
                        logger.warning(
                            ...
                            "(reason=%s, age=%.1fh) — re-add via `tiyazo auth add %s`",
                            _label,
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `tiyazo auth remove <provider> <N>` is stable across all source types.
    try:
        from tiyazo_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
        def _is_suppressed(_p, _s):  # type: ignore[misc]
            return False

    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, Tiyazo PKCE)
        ...
        # API-key vs OAuth is a user-visible choice at `tiyazo setup` ("Claude
        ...
        # Claude Code CLI, tiyazo_pkce creds from a previous OAuth login)
        ...
        # that masquerade.  Prefer ~/.tiyazo/.env over os.environ for the
        # same reason `_seed_from_env` does — that's the authoritative file
        # that `tiyazo setup` writes.
        _env_file = load_env()
        ...
        if api_key_path_explicit:
            ...
            retained = [
                ...
                if entry.source not in {"tiyazo_pkce", "claude_code"}
            ]
        ...
        from agent.anthropic_adapter import read_claude_code_credentials, read_tiyazo_oauth_credentials

        for source_name, creds in (
            ("tiyazo_pkce", read_tiyazo_oauth_credentials()),
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # `tiyazo auth add nous --label <name>`).  Fall back to the
        ...
        # Tiyazo auth store or env vars, so resolve them here.
        # Use refresh_if_expiring=False to avoid network calls during
        # pool loading / provider discovery.
        try:
            from tiyazo_cli.auth import resolve_qwen_runtime_credentials
        ...
        # MiniMax OAuth tokens live in ~/.tiyazo/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `tiyazo auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `tiyazo auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the Tiyazo auth store.  Without this gate the removal is instantly
        ...
        # Tiyazo owns its own Codex auth state — we do NOT auto-import from
        ...
        # When the user logs in via ``tiyazo model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``tiyazo auth list`` reflects the logged-in state and so the pool
        ...
        if isinstance(tokens, dict) and tokens.get("access_token"):
            ...
            from tiyazo_cli.auth import DEFAULT_XAI_OAUTH_BASE_URL
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Prefer ~/.tiyazo/.env over os.environ — the user's config file is the
    # authoritative source for Tiyazo credentials. Stale env vars from parent
    ...
    # Honour user suppression — `tiyazo auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.tiyazo/.env.
    ...
    try:
        from tiyazo_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
    ...
    def _secret_source_for_env(env_var: str) -> Optional[str]:
        try:
            from tiyazo_cli.env_loader import get_secret_source
    ...
    if provider == "openrouter":
        # Prefer ~/.tiyazo/.env over os.environ
...
def _prune_stale_seeded_entries(
    ...
    *,
    ...
    def _is_prunable(entry: PooledCredential) -> bool:
        ...
        # (e.g. an `tiyazo auth` command that confirmed the source is gone).
        if entry.source.startswith("env:"):
            return prune_env_sources
        # File-backed singletons (device-code OAuth, claude_code) and Tiyazo
