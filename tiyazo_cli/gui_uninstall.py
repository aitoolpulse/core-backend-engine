"""
Tiyazo Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``tiyazo desktop`` / ``tiyazo gui``)
     Built inside the agent checkout under ``$TIYAZO_HOME/tiyazo-agent/``:
       ...
       - ``$TIYAZO_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Tiyazo.app`` or ``~/Applications/Tiyazo.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Tiyazo`` (NSIS per-user)
...
the app name ("Tiyazo"), separate from ``$TIYAZO_HOME``:
  - macOS:   ``~/Library/Application Support/Tiyazo``
  - Windows: ``%APPDATA%\\Tiyazo``
  - Linux:   ``$XDG_CONFIG_HOME/Tiyazo`` (default ``~/.config/Tiyazo``)
...
``tiyazo uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(tiyazo_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return tiyazo_home / "tiyazo-agent"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "Tiyazo"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Tiyazo"
...
def source_built_gui_artifacts(tiyazo_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``tiyazo desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``tiyazo-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(tiyazo_home)
    ...
    return [
        ...
        tiyazo_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Tiyazo" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Tiyazo.app"),
            home / "Applications" / "Tiyazo.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Tiyazo).
            local_base / "Programs" / "Tiyazo",
            # Older / alternate layout some builds used.
            local_base / "tiyazo-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "Tiyazo")
    else:
        ...
        paths += [
            data_base / "applications" / "tiyazo.desktop",
            data_base / "applications" / "Tiyazo.desktop",
        ]
    return paths


def agent_is_installed(tiyazo_home: Path) -> bool:
    """Return True when a usable Python agent install exists under TIYAZO_HOME.
    ...
    if (agent_root / "tiyazo_cli").is_dir():
        return True
...
def gui_is_installed(tiyazo_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(tiyazo_home):
        if p.exists():
            return True
...
def gui_install_summary(tiyazo_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = tiyazo_home if tiyazo_home is not None else get_tiyazo_home()
...
def uninstall_gui(tiyazo_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``tiyazo-agent/tiyazo_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$TIYAZO_HOME``.
    ...
    if sys.platform.startswith("linux"):
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove tiyazo' or "
            "'sudo dnf remove tiyazo'). AppImage builds are a single file you "
