...
Plugins built on Tiyazo Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Tiyazo
...
:class:`~tiyazo_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Tiyazo' main config
...
handles every provider, fallback chain, and per-task override Tiyazo
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`tiyazo_cli.plugins.PluginContext`
