...
$TIYAZO_OSINT_CACHE/icij/ (default: ~/.cache/tiyazo-osint/icij/) and search
...
def _cache_dir() -> Path:
    base = os.environ.get("TIYAZO_OSINT_CACHE")
    if base:
        return Path(base) / "icij"
    return Path.home() / ".cache" / "tiyazo-osint" / "icij"
...
def _download(dest: Path, force: bool = False) -> Path:
    ...
    req = urllib.request.Request(
        BULK_URL,
        headers={"User-Agent": "tiyazo-agent osint-investigation skill"},
    )
...
def main() -> int:
    ...
    p.add_argument(
        ...
        type=Path,
        default=None,
        help="Override cache directory (default: $TIYAZO_OSINT_CACHE/icij or ~/.cache/tiyazo-osint/icij)",
    )
    p.add_argument(
        "--force-refresh",
        action="store_true",
