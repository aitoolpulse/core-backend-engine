#!/usr/bin/env bash
# Canonical test runner for tiyazo-agent. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.tiyazo/...)
#
# Usage:
...
# ── Activate venv ───────────────────────────────────────────────────────────
VENV=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.tiyazo/tiyazo-agent/venv"; do
...
if [ -f "$HOME/.tiyazo/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.tiyazo"
...
exec env -i \
  ...
  ${TIYAZO_RUN_SLOW_PET_TESTS:+TIYAZO_RUN_SLOW_PET_TESTS="$TIYAZO_RUN_SLOW_PET_TESTS"} \
