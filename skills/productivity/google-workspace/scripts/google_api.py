#!/usr/bin/env python3
"""Google Workspace API CLI for Tiyazo Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Tiyazo-facing JSON contract and falls back to the Python client
...
def _gws_binary() -> str | None:
    override = os.getenv("TIYAZO_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Tiyazo Agent")
