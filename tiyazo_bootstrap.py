"""Windows UTF-8 bootstrap for Tiyazo entry points.
...
should be imported at the very top of every Tiyazo entry point
(``tiyazo``, ``tiyazo-agent``, ``tiyazo-acp``, ``python -m gateway.run``,
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing Tiyazo modules.

    Tiyazo ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real Tiyazo source root
    ...
    """
    root = src_root or os.environ.get("TIYAZO_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``TIYAZO_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("TIYAZO_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import tiyazo_bootstrap``
# (or ``from tiyazo_bootstrap import apply_windows_utf8_bootstrap``) at
