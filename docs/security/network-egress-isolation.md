...
When running Tiyazo inside Docker, the default `network_mode: host` gives the
...
The Tiyazo [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
#
# Usage:
#   TIYAZO_UID=$(id -u) TIYAZO_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=tiyazo,tiyazo-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://tiyazo-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Tiyazo trust model and vulnerability reporting
