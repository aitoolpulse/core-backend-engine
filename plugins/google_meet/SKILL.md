...
```bash
tiyazo plugins enable google_meet
tiyazo meet install                 # pip deps + Chromium (transcribe only)
tiyazo meet install --realtime      # + pulseaudio-utils / brew blackhole+ffmpeg
tiyazo meet auth                    # optional; skips guest-lobby wait
tiyazo meet setup                   # preflight checks
```

`tiyazo meet install --realtime` prompts before running `sudo apt-get` (Linux)
...
#   Then set OPENAI_API_KEY or TIYAZO_MEET_REALTIME_KEY in ~/.tiyazo/.env
```
...
pip install playwright websockets && python -m playwright install chromium
tiyazo plugins enable google_meet
tiyazo meet node run --display-name my-mac    # persistent server
# copy the printed token

# on the gateway:
tiyazo meet node approve my-mac ws://<mac-ip>:18789 <token>
tiyazo meet node ping my-mac                   # confirm reachable
```

Run `tiyazo meet setup` to preflight local prereqs.

## Flow

1. **Join** — call `meet_join(url=..., mode=..., node=...)`. Returns immediately.
2. **Announce yourself** — no auto-consent. Say (in whatever channel the user is watching): "A Tiyazo agent bot is in this call taking notes."
...
- Guest mode sits in the lobby until a host admits. Warn the user; `tiyazo meet auth` avoids this.
- **Lobby timeout**: if the host doesn't admit the bot within 5 minutes (configurable via `TIYAZO_MEET_LOBBY_TIMEOUT` env), the bot leaves and `meet_status` reports `leaveReason: "lobby_timeout"`.
...
| `realtimeDevice` | Audio device name the bot is feeding (e.g. `tiyazo_meet_src`). |
...
Local:
```
$TIYAZO_HOME/workspace/meetings/<meeting-id>/transcript.txt
```
...
- Remote nodes use bearer-token auth; tokens are generated on the node (32 hex chars, persisted in `$TIYAZO_HOME/workspace/meetings/node_token.json`) and must be copied to the gateway via `tiyazo meet node approve`.
