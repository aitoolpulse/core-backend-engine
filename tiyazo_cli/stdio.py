...
on Japanese locales, etc.).  Tiyazo's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``tiyazo_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Tiyazo is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``TIYAZO_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("TIYAZO_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Tiyazo-managed Git install directories so
    ...
    # already-running shells don't see the change — which means tiyazo
    ...
    # install without Tiyazo-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Tiyazo's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Tiyazo's
    ``tiyazo config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Tiyazo (User env var in Windows Settings, or
    export in a PowerShell profile) and Tiyazo picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Tiyazo-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\tiyazo\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``tiyazo`` from right after install) retain their old PATH.

    Any subprocess Tiyazo spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Tiyazo-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "tiyazo", "git", "cmd"),
        os.path.join(local_appdata, "tiyazo", "git", "bin"),
        os.path.join(local_appdata, "tiyazo", "git", "usr", "bin"),
        # Tiyazo venv Scripts directory — host of the tiyazo.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes tiyazo, but harmless to include.
        os.path.join(local_appdata, "tiyazo", "tiyazo-agent", "venv", "Scripts"),
