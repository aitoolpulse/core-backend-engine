...
const logDir = join(process.env.TIYAZO_HOME?.trim() || join(homedir(), '.tiyazo'), 'logs')
...
// a real ~/.tiyazo (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('tiyazo-tui: parent lifecycle log unavailable\n')
    }
  }
}
