...
def snapshot_shutdown_context(received_signal: Any = None) -> Dict[str, Any]:
    ...
    * Wall-clock and monotonic timestamps for cross-correlating later phases
    ...
    try:
        tiyazo_home_str = os.environ.get("TIYAZO_HOME")
        if tiyazo_home_str:
            takeover_path = Path(tiyazo_home_str) / ".gateway-takeover.json"
            if takeover_path.exists():
                try:
                    ...
                    pass
            planned_stop_path = Path(tiyazo_home_str) / ".gateway-planned-stop.json"
...
def check_systemd_timing_alignment(drain_timeout: float) -> Optional[Dict[str, Any]]:
    ...
    try:
        # /proc/self/cgroup gives us "0::/user.slice/.../tiyazo-gateway.service"
    ...
    # constant in tiyazo_cli/gateway.py.
    headroom = 30.0
