...
- Skill helper paths:
  - `${TIYAZO_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${TIYAZO_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${TIYAZO_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client tiyazo
```
...
```bash
PUBLISH="${TIYAZO_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client tiyazo
```
...
```bash
DRIVE="${TIYAZO_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `tiyazo`)    |
