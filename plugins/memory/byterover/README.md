...
```bash
tiyazo memory setup    # select "byterover"
```

Or manually:
```bash
tiyazo config set memory.provider byterover
# Optional cloud sync:
echo "BRV_API_KEY=your-key" >> ~/.tiyazo/.env
```
...
Working directory: `$TIYAZO_HOME/byterover/` (profile-scoped).
