...
- ``tiyazo_cli.providers``        -- canonical provider identity + overlays
- ``tiyazo_cli.model_normalize``  -- per-provider name formatting
...
_TIYAZO_MODEL_WARNING = (
    "Nous Research Tiyazo 3 & 4 models are NOT agentic and are not designed "
    "for use with Tiyazo Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Nous Research Tiyazo 3 / Tiyazo 4 chat families.
# The previous substring check (`"tiyazo" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``tiyazo-brain:qwen3-14b-ctx16k`` that just
# happen to carry "tiyazo" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   NousResearch/Tiyazo-3-Llama-3.1-70B, tiyazo-4-405b, openrouter/tiyazo3:70b
# Negative examples it must NOT match:
#   tiyazo-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_NOUS_TIYAZO_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])tiyazo[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)


def is_nous_tiyazo_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Nous Tiyazo 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_NOUS_TIYAZO_NON_AGENTIC_RE.search(model_name))


def _check_tiyazo_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous Tiyazo 3/4 chat model."""
    if is_nous_tiyazo_non_agentic(model_name):
        return _TIYAZO_MODEL_WARNING
    return ""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``tiyazo config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from tiyazo_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'tiyazo model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'tiyazo doctor' — config issues detected:"
        ...
        from tiyazo_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from tiyazo_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    # =================================================================
        ...
        try:
            from tiyazo_cli.config import load_config
            from tiyazo_cli.moa_config import exact_moa_preset_name, normalize_moa_config
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from tiyazo_cli.providers import resolve_user_provider as _ruser
    ...
    # tiyazo_cli.runtime_provider.resolve_runtime_provider; without the strip,
    ...
    if target_provider in {"opencode-zen", "opencode-go"} and isinstance(base_url, str):
        from tiyazo_cli.models import normalize_opencode_base_url
    ...
    tiyazo_warn = _check_tiyazo_model_warning(new_model)
    if tiyazo_warn:
        warnings.append(tiyazo_warn)
...
def _extra_headers_from_config(entry: Any) -> dict[str, str]:
    if not isinstance(entry, dict):
        return {}
    from tiyazo_cli.config import normalize_extra_headers
...
def prewarm_picker_cache_async() -> Optional["_threading.Thread"]:
    ...
    def _warm() -> None:
        try:
            from tiyazo_cli.inventory import load_picker_context
...
def list_authenticated_providers(
    ...
    *,
    ...
    Uses the curated model lists from tiyazo_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from tiyazo_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by tiyazo provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "nous" pulls from the remote model-catalog manifest published at
    # https://tiyazo-agent.nousresearch.com/docs/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring a Tiyazo release. Falls back to the in-repo
    # _PROVIDER_MODELS["nous"] snapshot when the manifest is unreachable.
    curated["nous"] = get_curated_nous_model_ids()
    # Ollama Cloud uses dynamic discovery (no static curated list)
    if "ollama-cloud" not in curated:
        from tiyazo_cli.models import fetch_ollama_cloud_models
    ...
    if "lmstudio" not in curated and (
        ...
        from tiyazo_cli.models import fetch_lmstudio_models
    ...
    # --- 1. Check Tiyazo-mapped providers ---
    from tiyazo_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from tiyazo_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    for tiyazo_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        _alias_target = _PROVIDER_ALIAS_TABLE.get(tiyazo_id)
        if (
            _alias_target
            and _alias_target != tiyazo_id
        ...
        pconfig = PROVIDER_REGISTRY.get(tiyazo_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (TIYAZO_OVERLAYS) with proper auth store checking.
        ...
        if not has_creds:
            try:
                from tiyazo_cli.auth import _load_auth_store
                store = _load_auth_store()
                if store and store.get("credential_pool", {}).get(tiyazo_id):
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
        ...
        # /model picker sees the SAME list `tiyazo model` would build, with
        # disk caching to keep the picker open snappy. Falls back to the
        # curated static list when the live fetcher returns nothing.
        model_ids = cached_provider_model_ids(tiyazo_id)
        if not model_ids:
            model_ids = curated.get(tiyazo_id, [])
            if tiyazo_id in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(tiyazo_id, model_ids)
        total = len(model_ids)
        if tiyazo_id in _UNCAPPED_PICKER_PROVIDERS:
    ...
    # --- 2. Check Tiyazo-only providers (nous, openai-codex, copilot, opencode-go) ---
    from tiyazo_cli.providers import TIYAZO_OVERLAYS
    from tiyazo_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Tiyazo provider ID.
    # TIYAZO_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Tiyazo IDs ("copilot").
    _mdev_to_tiyazo = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in TIYAZO_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Tiyazo slug — e.g. "github-copilot" → "copilot"
        tiyazo_slug = _mdev_to_tiyazo.get(pid, pid)
        if tiyazo_slug.lower() in seen_slugs:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(tiyazo_slug)
        elif overlay.extra_env_vars:
            has_creds = any(os.environ.get(ev) for ev in overlay.extra_env_vars)
        # Also check api_key_env_vars from PROVIDER_REGISTRY for api_key auth_type
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, tiyazo_slug):
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or tiyazo_slug in providers_store):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(tiyazo_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", tiyazo_slug, exc)
        ...
        # configured.
        if not has_creds and tiyazo_slug == "anthropic":
            try:
                ...
                if (tiyazo_creds and tiyazo_creds.get("accessToken")) or \
        ...
        if tiyazo_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = cached_provider_model_ids(tiyazo_slug)
        # For aws_sdk providers (bedrock), use live discovery so the list
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        elif overlay.auth_type == "aws_sdk":
            try:
                _ids = cached_provider_model_ids(tiyazo_slug)
                model_ids = _ids if _ids else (curated.get(tiyazo_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(tiyazo_slug, []) or curated.get(pid, [])
        elif tiyazo_slug == "nous":
            # Nous serves a large live /v1/models catalog (vendor-prefixed
            # models from many providers, returned alphabetically). The
            # `tiyazo model` picker deliberately shows ONLY the curated agentic
            ...
            try:
                ...
                from tiyazo_cli.auth import get_provider_auth_state as _nous_state
            ...
            if not model_ids:
                model_ids = curated.get(tiyazo_slug, []) or curated.get(pid, [])
                if tiyazo_slug in _MODELS_DEV_PREFERRED:
                    model_ids = _merge_with_models_dev(tiyazo_slug, model_ids)
        total = len(model_ids)
        if tiyazo_slug in _UNCAPPED_PICKER_PROVIDERS:
            top = model_ids  # Aggregator: show full catalog regardless of max_models
        else:
            top = model_ids[:max_models] if max_models is not None else model_ids
        ...
        results.append({
            "slug": tiyazo_slug,
            "name": get_label(tiyazo_slug),
            "is_current": tiyazo_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "tiyazo",
        ...
        seen_slugs.add(tiyazo_slug.lower())
        _record_builtin_endpoint(tiyazo_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or TIYAZO_OVERLAYS (keeps /model in sync
    # with `tiyazo model`).
    try:
        from tiyazo_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is Tiyazo's canonical write key (matches
            ...
            # Tiyazo writes ``models:`` as a dict keyed by model id, but older
            ...
            if should_probe:
                try:
                    from tiyazo_cli.models import fetch_api_models
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # Tiyazo's own writer uses; a hyphen variant is accepted
            ...
            # active model. Tiyazo's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, tiyazo_cli/config.py) already consume that dict.
...
def _prepend_moa_picker_provider(providers: List[dict], current_provider: str = "") -> List[dict]:
    ...
    """
    try:
        from tiyazo_cli.inventory import _moa_provider_row
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`tiyazo_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from tiyazo_cli.models import fetch_openrouter_models
