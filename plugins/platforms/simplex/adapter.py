"""SimpleX Chat platform adapter (Tiyazo plugin).
...
This adapter ships as a Tiyazo platform plugin under
``plugins/platforms/simplex/``. The Tiyazo plugin loader scans the
...
Optional environment variables:
    ...
    TIYAZO_SIMPLEX_TEXT_BATCH_DELAY
...
discoverable and ``tiyazo setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Tiyazo — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("TIYAZO_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``tiyazo setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.tiyazo/.env`` via
    ``tiyazo_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "tiyazo_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.tiyazo/.env"
        )
        return
