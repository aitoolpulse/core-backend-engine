"""
Dump command for tiyazo CLI.

Outputs a compact, plain-text summary of the user's Tiyazo setup
...
from tiyazo_cli.config import get_tiyazo_home, get_env_path, get_project_root, load_config
...
def _dotenv_key_names() -> set[str]:
    """Return the set of env-var names assigned a non-empty value in ~/.tiyazo/.env.
    ...
    exports. ``tiyazo debug share`` runs in a terminal, so ``os.getenv`` reflects
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.tiyazo_build_sha`` by the Dockerfile's
    ``TIYAZO_GIT_SHA`` build-arg (see ``tiyazo_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``tiyazo dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from tiyazo_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(tiyazo_home: Path) -> int:
...
def _cron_summary(tiyazo_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = tiyazo_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"tiyazo_home:      {display_tiyazo_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # Set in this (shell) process but absent from ~/.tiyazo/.env: a managed
        ...
        # A credential added via `tiyazo auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `tiyazo auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["tiyazo-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(tiyazo_home)}")
    lines.append(f"  skills:             {_count_skills(tiyazo_home)}")
