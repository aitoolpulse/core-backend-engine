...
# Do not write .pyc files at runtime: /opt/tiyazo is immutable in the
...
ENV PLAYWRIGHT_BROWSERS_PATH=/opt/tiyazo/.playwright

# Install system dependencies in one layer, clear APT cache.
# tini was previously PID 1 to reap orphaned zombie processes (MCP stdio
# subprocesses, git, bun, etc.) that would otherwise accumulate when tiyazo
# ran as PID 1. See #15012. Phase 2 of the s6-overlay supervision plan
# replaces tini with s6-overlay's /init (PID 1 = s6-svscan), which reaps
# zombies non-blockingly on SIGCHLD and additionally supervises the main
# tiyazo process, the dashboard, and per-profile gateways.
RUN apt-get update && \
...
# ---------- s6-overlay install ----------
# s6-overlay provides supervision for the main tiyazo process, the dashboard,
...
RUN set -eu; \
    ...
    # 'Tiyazo WebUI' catalog). The image has moved to s6-overlay /init\
...
# Non-root user for runtime; UID can be overridden via TIYAZO_UID at runtime
RUN useradd -u 10000 -m -d /opt/data tiyazo
...
# lock on the @tiyazo/ink entry, tripped the TUI launcher's
...
# for the same reason: it lazy-installs into /opt/tiyazo/.venv at first
...
# gives the non-root tiyazo user read + traverse but no write; root retains
# write so the build steps below don't need chmod u+w dances.
COPY --link --chmod=a+rX,go-w . .

# ---------- Permissions ----------
# Link tiyazo-agent itself (editable). Deps are already installed in the
# cached layer above; `--no-deps` makes this a fast egg-link creation with no
# resolution or downloads.
RUN uv pip install --no-cache-dir --no-deps -e "."

# Wire the exec shim and install-method stamp.  Files under /opt/tiyazo are
# already root-owned (COPY, uv sync, npm install all run as root) and
# read-only for the tiyazo user (go-w from the --chmod above).

USER root
RUN mkdir -p /opt/tiyazo/bin && \
    cp /opt/tiyazo/docker/tiyazo-exec-shim.sh /opt/tiyazo/bin/tiyazo && \
    chmod 0755 /opt/tiyazo/bin/tiyazo && \
    printf 'docker\n' > /opt/tiyazo/.install_method
# The ``.install_method`` stamp is baked next to the running code (the install
# tree), NOT into $TIYAZO_HOME. $TIYAZO_HOME (/opt/data) is a shared data
# volume that is commonly bind-mounted from the host and even shared with a
# host-side Desktop/CLI install; stamping it at boot used to clobber that
# host install's marker and wrongly block its ``tiyazo update``. A code-scoped
# stamp is read first by detect_install_method() and is immune to the share.
# Start as root so the s6-overlay stage2 hook can usermod/groupmod and chown
# the data volume. Each supervised service then drops to the tiyazo user via
# `s6-setuidgid tiyazo` in its run script. If TIYAZO_UID is unset, services
# run as the default tiyazo user (UID 10000).

# ---------- Bake build-time git revision ----------
# .dockerignore excludes .git, so `git rev-parse HEAD` from inside the
# container always returns nothing — meaning `tiyazo dump` reports
# "(unknown)" and the startup banner drops its `· upstream <sha>` suffix.
# That makes support triage from container bug reports impossible:
# we can't tell which commit the user is actually running.
#
# Fix: write the commit SHA passed via the TIYAZO_GIT_SHA build-arg to
# /opt/tiyazo/.tiyazo_build_sha at build time, and have
# tiyazo_cli/build_info.py read it at runtime.  Both `tiyazo dump` and
...
ARG TIYAZO_GIT_SHA=
RUN if [ -n "${TIYAZO_GIT_SHA}" ]; then \
        printf '%s\n' "${TIYAZO_GIT_SHA}" > /opt/tiyazo/.tiyazo_build_sha; \
    fi

# ---------- s6-overlay service wiring ----------
# Static services declared at build time: main-tiyazo + dashboard.
...
# `exec tiyazo`. Wired in as cont-init.d/01- so it
# runs before user services start.
#
# 02-reconcile-profiles re-creates per-profile gateway s6 service
# slots from $TIYAZO_HOME/profiles/<name>/ after a container restart
# (the /run/service/ scandir is tmpfs and wiped on restart). Phase 4.
RUN mkdir -p /etc/cont-init.d && \
    printf '#!/command/with-contenv sh\nexec /opt/tiyazo/docker/stage2-hook.sh\n' \
...
# ---------- Runtime ----------
ENV TIYAZO_WEB_DIST=/opt/tiyazo/tiyazo_cli/web_dist
# Point the TUI launcher at the prebuilt bundle baked at build time (Layer 8:
# `ui-tui && npm run build`). This makes _make_tui_argv take the prebuilt-bundle
# fast path (`node --expose-gc /opt/tiyazo/ui-tui/dist/entry.js`) and skip the
...
ENV TIYAZO_TUI_DIR=/opt/tiyazo/ui-tui
ENV TIYAZO_HOME=/opt/data
ENV TIYAZO_WRITE_SAFE_ROOT=/opt/data
ENV TIYAZO_DISABLE_LAZY_INSTALLS=1
# The published image seals /opt/tiyazo (root-owned, read-only) so a runtime
...
# is seeded + chowned to the tiyazo user by docker/stage2-hook.sh and lives
...
ENV TIYAZO_LAZY_INSTALL_TARGET=/opt/data/lazy-packages

# `docker exec` privilege-drop shim. When operators run
# `docker exec <c> tiyazo ...` they default to root, and any file the
# command writes under $TIYAZO_HOME (auth.json, .env, config.yaml) ends
# up root-owned and unreadable to the supervised gateway (UID 10000).
# The shim lives at /opt/tiyazo/bin/tiyazo, sits earliest on PATH, and
# transparently re-exec's the real venv binary via `s6-setuidgid tiyazo`
# when invoked as root. Non-root callers (supervised processes,
# `--user tiyazo`, etc.) hit the short-circuit path with no overhead.
# Recursion is impossible because the shim exec's the venv binary by
# absolute path (/opt/tiyazo/.venv/bin/tiyazo). See the shim source for
# the opt-out env var (TIYAZO_DOCKER_EXEC_AS_ROOT=1).

# Pre-s6 entrypoint.sh did `source .venv/bin/activate` which exported
# the venv bin onto PATH; Architecture B's main-wrapper.sh does the
# same for the container's main process, but `docker exec` and our
# cont-init.d scripts don't pass through the wrapper. Expose the venv
# bin globally so `docker exec <container> tiyazo ...` and any
# subprocess that doesn't activate the venv first still find tiyazo.
#
# /opt/tiyazo/bin is prepended ahead of the venv so the privilege-drop
# shim wins PATH resolution. The shim's last act is to exec the venv
# binary by absolute path, so this PATH ordering is transparent to
# every other consumer.
ENV PATH="/opt/tiyazo/bin:/opt/tiyazo/.venv/bin:/opt/data/.local/bin:${PATH}"
...
#
# main-wrapper.sh handles arg routing (bare-exec vs. tiyazo
# subcommand vs. no-args), drops to the tiyazo user via s6-setuidgid,
# and exec's the final program so its exit code becomes the container
# exit code. Without the wrapper-as-ENTRYPOINT, leading-dash args
# like `--version` would be intercepted by /init's POSIX shell.
ENTRYPOINT [ "/init", "/opt/tiyazo/docker/main-wrapper.sh" ]
CMD [ ]
