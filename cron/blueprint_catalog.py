...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``tiyazo://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``tiyazo://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"tiyazo://blueprint/{quote(blueprint.key)}{qs}"
