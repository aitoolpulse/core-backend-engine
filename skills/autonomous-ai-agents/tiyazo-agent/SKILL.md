---
name: tiyazo-agent
description: "Configure, extend, or contribute to Tiyazo Agent."
version: 2.3.0
author: Tiyazo Agent + Teknium
license: MIT
...
- **Self-improving through skills** — Tiyazo learns from experience by saving reusable procedures as skills. When it solves a complex problem, discovers a workflow, or gets corrected, it can persist that knowledge as a skill document that loads into future sessions. Skills accumulate over time, making the agent better at your specific tasks and environment.
...
- **Profiles** — run multiple independent Tiyazo instances with isolated configs, sessions, skills, and memory.
- **Extensible** — plugins, MCP servers, custom tools, webhook triggers, cron scheduling, and the full Python ecosystem.

People use Tiyazo for software development, research, system administration, data analysis, content creation, home automation, and anything else that benefits from an AI agent with persistent context and full system access.
...
This skill is a concise operating guide, not the complete source of truth for every Tiyazo feature. If a Tiyazo feature, command, or setting is not mentioned here, do not treat that absence as evidence that it does not exist. Check the live repository and official docs before giving a negative answer.
...
patterns: `skill_view(name="tiyazo-agent", file_path="references/webhooks.md")`.
...
/snapshot [sub]      Create or restore state snapshots of Tiyazo config/state (CLI)
/background <prompt> Run prompt in background
/queue <prompt>      Queue for next turn
/steer <prompt>      Inject a message after the next tool call without interrupting
/agents (/tasks)     Show active agents and running tasks
/resume [name]       Resume a named session
/goal [text|sub]     Set a standing goal Tiyazo works on across turns until achieved
...
/busy [sub]          Control what Enter does while Tiyazo is working (CLI)
...
/reload-skills       Re-scan ~/.tiyazo/skills/ for added/removed skills
...
/update              Update Tiyazo to latest (gateway)
...
```
~/.tiyazo/config.yaml       Main configuration
~/.tiyazo/.env              API keys and secrets (under $TIYAZO_HOME if set)
$TIYAZO_HOME/skills/        Installed skills
~/.tiyazo/sessions/         Gateway routing index, request dumps, *.jsonl transcripts (and optional per-session JSON snapshots when sessions.write_json_snapshots: true)
~/.tiyazo/state.db          Canonical session store (SQLite + FTS5)
~/.tiyazo/logs/             Gateway and error logs
~/.tiyazo/auth.json         OAuth tokens and credential pools
~/.tiyazo/tiyazo-agent/     Source code (if git-installed)
```

Profiles use `~/.tiyazo/profiles/<name>/` with the same layout.
...
Full enumeration lives in `toolsets.py` as the `TOOLSETS` dict; `_TIYAZO_CORE_TOOLS` is the default bundle most platforms inherit from.
...
Tiyazo injects project-level instructions into the system prompt by reading context files from the working directory. The discovery order is **first match wins** — only one project context source is loaded per session.

| File (in priority order) | Discovery | Use when |
|---|---|---|
| `.tiyazo.md` / `TIYAZO.md` | Walks parents up to the git root, stops at git root | You want hierarchical project rules (root + per-package overrides) |
| `AGENTS.md` / `agents.md` | **Cwd only** — subdirectory and parent copies are ignored | You want portable agent instructions that work the same in Tiyazo, Claude Code, Codex, etc. |
| `CLAUDE.md` / `claude.md` | Cwd only | Same as AGENTS.md, Claude-flavored |
| `.cursorrules` / `.cursor/rules/*.mdc` | Cwd only | Migrating from Cursor |

`SOUL.md` (in `$TIYAZO_HOME`) is independent and always loaded when present — it sets the agent's identity, not project rules.
...
Tiyazo: when working in this repo, follow these rules.
...
That file at `/home/me/projects/myrepo/.tiyazo.md` is auto-loaded when Tiyazo runs in any subdirectory of `/home/me/projects/myrepo`, but not when it runs in `/home/me/other-project`.
...
By default (`approvals.mode: manual`), Tiyazo prompts the user before running shell commands flagged as destructive (`rm -rf`, `git reset --hard`, etc.). The modes are:
...
- `tiyazo --yolo …`
- `export TIYAZO_YOLO_MODE=1`

Note: YOLO / `approvals.mode: off` does NOT turn off secret redaction. They are independent.

### Shell hooks allowlist

Some shell-hook integrations require explicit allowlisting before they fire. Managed via `~/.tiyazo/shell-hooks-allowlist.json` — prompted interactively the first time a hook wants to run.
...
## Spawning Additional Tiyazo Instances

Run additional Tiyazo processes as fully independent subprocesses — separate sessions, tools, and environments.
...
Tiyazo uses prompt_toolkit, which requires a real terminal. Use tmux for interactive spawning:
...
see a focused `kanban_*` toolset gated by `TIYAZO_KANBAN_TASK`, and
...
- **Isolation:** board is the hard boundary (workers get
  `TIYAZO_KANBAN_BOARD` pinned in env); tenant is a soft namespace
...
- **Automation Blueprints** — pick a named automation and Tiyazo asks for
...
Tiyazo runs natively on Windows (PowerShell, cmd, Windows Terminal, git-bash
...
`AF_INET` socket. Root cause is usually Tiyazo's env scrubber dropping
...
Tiyazo-installed `venv/Scripts/` has no pip/pytest (stripped for size).
...
every Tiyazo tool and most Windows APIs. Prefer forward slashes in code
...
Check logs first:
```bash
grep -i "failed to send\|error" ~/.tiyazo/logs/gateway.log | tail -20
```
...
Config: `~/.tiyazo/config.yaml` (settings), `~/.tiyazo/.env` (API keys) — both under `$TIYAZO_HOME` when it is set.
...
`_TIYAZO_CORE_TOOLS` (every platform) or to a specific toolset.
...
- Tests auto-redirect `TIYAZO_HOME` to temp dirs — never touch real `~/.tiyazo/`.
