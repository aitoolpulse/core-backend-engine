...
The design is **Tiyazo-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
tiyazo skills install official/security/unbroker
```

Then start a new Tiyazo session and drive it (below). The skill works zero-config; a few optional
...
Drive it from a Tiyazo session:
...
- **PII handling.** Dossiers live under the Tiyazo home directory (`0600`, optionally
