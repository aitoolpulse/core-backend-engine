...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Tiyazo Agent\",
    \"comments\": [
...
```bash
source "${TIYAZO_HOME:-$HOME/.tiyazo}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Tiyazo Agent. Code looks clean — good test coverage, no security concerns."
