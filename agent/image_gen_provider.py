...
as ``kind: backend``) or ``~/.tiyazo/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class ImageGenProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``tiyazo tools``. Defaults to ``name.title()``."""
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        """Return catalog entries for ``tiyazo tools`` model picker.
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        """Return provider metadata for the ``tiyazo tools`` picker.
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        model knows when ``image_url`` is honored. Used by ``tiyazo tools``
...
def _images_cache_dir() -> Path:
    """Return ``$TIYAZO_HOME/cache/images/``, creating parents as needed."""
    from tiyazo_constants import get_tiyazo_home

    path = get_tiyazo_home() / "cache" / "images"
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$TIYAZO_HOME/cache/images/``.
...
def save_url_image(
    ...
    *,
    ...
    """Download an image URL and write it under ``$TIYAZO_HOME/cache/images/``.
