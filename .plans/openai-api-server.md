# OpenAI-Compatible API Server for Tiyazo Agent
...
SSE streaming. By exposing this endpoint, tiyazo-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  tiyazo-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.tiyazo/.env`
2. Run `tiyazo gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with tiyazo-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns tiyazo-agent as a model) |
...
Authorization: Bearer tiyazo-api-key-here
...
But tiyazo-agent sessions have persistent state (memory, skills, tool context).
...
The agent's `run_conversation()` is synchrotiyazo and returns the full response.
...
Frontends send `"model": "tiyazo-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured tiyazo-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of tiyazo-agent's core prompt
...
Once implemented, tiyazo-agent works as a drop-in backend for:
