...
``/opt/hermes``), ``<root>`` is ``TIYAZO_HOME``. Legacy env-var
...
def _resolve_tiyazo_argv() -> list[str]:
    ...
    tiyazo_bin = _safe_which_no_cwd("hermes") if _IS_WINDOWS else shutil.which("hermes")
    if tiyazo_bin:
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled HERMES_PROFILE values like "tiyazo-system"
