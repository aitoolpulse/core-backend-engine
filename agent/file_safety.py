...
def _tiyazo_root_path() -> Path:
    """Resolve the Tiyazo root dir (always the parent of any profile, never per-profile)."""
    try:
...
def get_safe_write_roots() -> set[str]:
    """Return resolved TIYAZO_WRITE_SAFE_ROOT paths. Supports multiple directories
    separated by ``os.pathsep`` (``:`` on Unix, ``;`` on Windows).
    E.g., ``/opt/data:/var/www/html`` on Unix, ``C:\\data;D:\\www`` on Windows."""
    env = os.getenv("TIYAZO_WRITE_SAFE_ROOT", "")
    if not env:
        return set()
    roots: set[str] = set()
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied Tiyazo path.

    Three categories are blocked:

      * Internal Tiyazo cache files under ``TIYAZO_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under TIYAZO_HOME and the global Tiyazo
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.tiyazo/.env`` and exfiltrate the file. The read-deny exists
    ...
    # Resolve BOTH the active TIYAZO_HOME (profile-aware) AND the global
    # Tiyazo root so credential stores at <root>/auth.json etc. are also
    ...
    for hd in tiyazo_dirs:
        ...
        for blocked in blocked_dirs:
            try:
                ...
                continue
            return (
                f"Access denied: {path} is an internal Tiyazo cache file "
    ...
    for hd in tiyazo_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a Tiyazo credential store "
    ...
    for hd in tiyazo_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the Tiyazo MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a Tiyazo MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
...
def raise_if_read_blocked(path: str) -> None:
    """Raise ``ValueError`` if ``path`` is a denied Tiyazo read (see
...
#
# Tiyazo profiles are separate TIYAZO_HOME dirs under
...
# Profile-scoped directories under TIYAZO_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    """Return the active profile name derived from TIYAZO_HOME.
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside Tiyazo scope, or is inside
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    Tiyazo entirely. Caller is expected to surface the warning to the
    ...
    return (
        ...
        f"belongs to Tiyazo profile {info['target_profile']!r}, but the "
...
#
#   <TIYAZO_HOME>/profiles/<name>/sandboxes/<backend>/<task>/home/.tiyazo/...
#
...
# ``…/sandboxes/<backend>/<task>/home/.tiyazo/…`` segment and warns
# regardless of which Tiyazo profile is active. It does NOT cover the
# inner-container case where the bind mount strips the ``sandboxes/`` prefix
# (the agent's view inside the container is plain ``/root/.tiyazo/...``);
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    """Return the index of the inner ``.tiyazo`` part in a sandbox-mirror path.

    Matches ``…/sandboxes/<backend>/<task>/home/.tiyazo/…`` and returns the
    index where the inner Tiyazo-state portion starts. Returns ``None`` for
    paths that do not contain the sandbox-mirror shape.
    """
    for i, part in enumerate(parts):
        if part != "sandboxes":
            continue
        # Need at least: sandboxes / <backend> / <task> / home / .tiyazo / <thing>
        if i + 5 >= len(parts):
            continue
        if parts[i + 3] == "home" and parts[i + 4] == ".tiyazo":
            return i + 4
    return None


def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative Tiyazo state.

    Returns ``None`` when the path does not match the sandbox-mirror shape.
    Otherwise returns a dict with:

      * ``target_path``: the resolved path string
      * ``mirror_root``: the ``…/sandboxes/<backend>/<task>/home/.tiyazo``
        prefix (so callers can show users which sandbox owns the mirror)
      * ``inner_path``: the portion under the mirror's ``.tiyazo`` (what the
        agent likely meant to address on the host)

    Detection is path-shape-only — does not require any Tiyazo resolver to
    succeed, so it works correctly even when called from contexts where
    TIYAZO_HOME resolution would be ambiguous.
    """
    try:
        target = Path(os.path.expanduser(str(path))).resolve()
    except (OSError, RuntimeError):
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"Writes here land on a copy that the host Tiyazo process never "
        f"reads — the authoritative file is likely {info['inner_path']!r} "
        f"under the real TIYAZO_HOME. Use the host-side tool for "
...
# full ``…/sandboxes/<backend>/<task>/home/.tiyazo/…`` prefix on the host.
# But when file tools execute *inside* the container the bind-mount strips
# that prefix: the agent sees plain ``/root/.tiyazo/…``.  The root:root
...
def classify_container_mirror_target(
    ...
    under the mirror prefix. Otherwise returns:
      ...
      * ``inner_path``: portion under the mirror root (what the agent
        likely meant to address in the host TIYAZO_HOME)
    """
...
def get_container_mirror_warning(
    ...
    sandbox mirror of authoritative Tiyazo state.
    ...
    return (
        ...
        f"bind-mounted home — a per-task mirror that the host Tiyazo "
        f"process never reads. The authoritative file is "
        f"{info['inner_path']!r} under the real TIYAZO_HOME. Use the "
