---
name: honcho
description: Configure and use Honcho memory with Tiyazo -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.
version: 2.0.0
author: Tiyazo Agent
license: MIT
...
# Honcho Memory for Tiyazo

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Tiyazo profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/tiyazo#running-honcho-locally-with-tiyazo

### Verify

```bash
tiyazo honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Tiyazo profile's AI peer
...
Honcho models conversations as interactions between **peers**. Tiyazo creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Tiyazo instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Tiyazo run |
| `global` | Single session across all directories |

Manual override: `tiyazo honcho map my-project-name`
...
Each Tiyazo profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
```bash
tiyazo profile create coder --clone
# creates host block tiyazo.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `tiyazo.coder` host block in `honcho.json`
...
```bash
tiyazo honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Tiyazo when Honcho memory is active.
...
Config file: `$TIYAZO_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `tiyazo honcho setup`. Ensure `memory.provider: honcho` is in `~/.tiyazo/config.yaml`.

### Memory not persisting across sessions
Check `tiyazo honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `tiyazo profile create <name> --clone`. For existing profiles: `tiyazo honcho sync`.
...
| Command | Description |
|---------|-------------|
| `tiyazo honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `tiyazo honcho status` | Show resolved config, connection test, peer info for active profile |
| `tiyazo honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `tiyazo honcho disable` | Disable Honcho for the active profile |
| `tiyazo honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `tiyazo honcho peers` | Show peer identities across all profiles |
| `tiyazo honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `tiyazo honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `tiyazo honcho sessions` | List known directory-to-session-name mappings |
| `tiyazo honcho map <name>` | Map current working directory to a Honcho session name |
| `tiyazo honcho identity` | Seed AI peer identity or show both peer representations |
| `tiyazo honcho sync` | Create host blocks for all Tiyazo profiles that don't have one yet |
| `tiyazo honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Tiyazo + Honcho |
| `tiyazo memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `tiyazo memory status` | Show active memory provider and config |
| `tiyazo memory off` | Disable external memory provider |
