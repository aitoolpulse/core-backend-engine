...
- smoke-test a server with the FastMCP CLI before wiring it into Tiyazo or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Tiyazo. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.tiyazo/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.tiyazo/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Tiyazo integration, either:

- configure the server in `~/.tiyazo/config.yaml` using the `native-mcp` skill, or
...
### Tiyazo cannot see the deployed server

The server-building part may be correct while the Tiyazo config is not. Load the `native-mcp` skill and configure the server in `~/.tiyazo/config.yaml`, then restart Tiyazo.
