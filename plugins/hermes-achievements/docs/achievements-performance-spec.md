# Tiyazo Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: tiyazo-achievements plugin
...
### `GET /api/plugins/tiyazo-achievements/achievements`
...
### `POST /api/plugins/tiyazo-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/tiyazo-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/tiyazo-achievements/overview`
...
- `~/.tiyazo/plugins/tiyazo-achievements/`
