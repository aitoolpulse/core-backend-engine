...
Tiyazo middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing Tiyazo'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Tiyazo calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Tiyazo retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Tiyazo guardrails,
...
- `telemetry_schema_version`: currently `tiyazo.observer.v1`
- `middleware_schema_version`: currently `tiyazo.middleware.v1`
...
Tiyazo stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Tiyazo runs
them as a nested chain in registration order. Middleware failures are fail-open:
Tiyazo logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, Tiyazo applies middleware in this order:
...
For each tool call, Tiyazo applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Tiyazo pre-execution path against the effective arguments:
...
```bash
tiyazo plugins enable <plugin-name>
```

For isolated local testing, use one `TIYAZO_HOME` for plugin enablement and the
agent run:

```bash
export TIYAZO_HOME=/tmp/tiyazo-middleware-test
mkdir -p "$TIYAZO_HOME"
tiyazo plugins enable <plugin-name>
tiyazo chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run tiyazo plugins enable <plugin-name>
uv run tiyazo chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["tiyazo_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/tiyazo-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape Tiyazo expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Tiyazo treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Tiyazo preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Tiyazo does not convert downstream
