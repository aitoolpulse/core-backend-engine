"""CLI entry point for the tiyazo-agent ACP adapter.

Loads environment variables from ``~/.tiyazo/.env``, configures logging
...
def _load_env() -> None:
    """Load .env from TIYAZO_HOME (default ``~/.tiyazo``)."""
    from tiyazo_cli.env_loader import load_tiyazo_dotenv

    tiyazo_home = get_tiyazo_home()
    loaded = load_tiyazo_dotenv(tiyazo_home=tiyazo_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", tiyazo_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.tiyazo/node/ "
...
def _run_setup() -> None:
    from tiyazo_cli.main import main as tiyazo_main
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting tiyazo-agent ACP adapter")
