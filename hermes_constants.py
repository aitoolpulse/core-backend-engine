...
def get_hermes_home() -> Path:
    ...
    ``hermes_cli/kanban_db.py``).  See https://github.com/aitoolpulse/core-backend-engine/issues/18594.
    """
...
def heal_hermes_managed_node() -> bool:
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "TIYAZO_HOME": str(get_hermes_home())},
            capture_output=True,
            timeout=300,
            check=False,
        )
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    ...
    hermes_home = get_hermes_home_override() or (env or {}).get("TIYAZO_HOME") or os.getenv("TIYAZO_HOME")
    if not hermes_home:
        return None
