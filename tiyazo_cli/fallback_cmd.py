"""
tiyazo fallback — manage the fallback provider chain.
...
Subcommands:
  tiyazo fallback [list]   Show the current fallback chain (default when no subcommand)
  tiyazo fallback add      Pick provider + model via the same picker as `tiyazo model`,
                           then append the selection to the chain
  tiyazo fallback remove   Pick an entry to delete from the chain
  tiyazo fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.tiyazo/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from tiyazo_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `tiyazo model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  tiyazo fallback add")
        print()
        return
    ...
    print("  Docs: https://tiyazo-agent.nousresearch.com/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `tiyazo model`, then append the selection to the chain."""
    from tiyazo_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `tiyazo model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `tiyazo fallback list` to view, or `tiyazo fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``tiyazo fallback [subcommand]``."""
