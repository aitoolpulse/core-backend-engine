"""tiyazo webhook — manage dynamic webhook subscriptions from the CLI.

Usage:
    tiyazo webhook subscribe <name> [options]
    tiyazo webhook list
    tiyazo webhook remove <name>
    tiyazo webhook test <name> [--payload '{"key": "value"}']

Subscriptions persist to ~/.tiyazo/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _tiyazo_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: tiyazo gateway run
"""
...
def webhook_command(args):
    """Entry point for 'tiyazo webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: tiyazo webhook {subscribe|list|remove|test}")
        print("Run 'tiyazo webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print("  The gateway must be running to receive events (tiyazo gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: tiyazo webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from tiyazo webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (tiyazo gateway run)")
