"""ACP agent server — exposes Tiyazo Agent via the Agent Client Protocol."""
...
from tools.approval import (
    reset_tiyazo_interactive_context,
    set_tiyazo_interactive_context,
)

logger = logging.getLogger(__name__)

try:
    from tiyazo_cli import __version__ as TIYAZO_VERSION
except Exception:
    TIYAZO_VERSION = "0.0.0"
...
def _path_from_file_uri(uri: str) -> Path | None:
    ...
    /mnt/<drive>/... so Tiyazo running in WSL can read it.
    """
    raw = (uri or "").strip()
    if not raw:
        return None
    ...
    if path is None:
        return [{
            ...
            "text": _format_resource_text(
                ...
                name=name,
                title=title,
                body="[Resource link only; Tiyazo cannot read non-file ACP resource URIs directly.]",
            ),
        }]
...
def _content_blocks_to_openai_user_content(
    ...
    """Convert ACP prompt blocks into a Tiyazo/OpenAI-compatible user content payload."""
...
class TiyazoACPAgent(acp.Agent):
    """ACP Agent implementation wrapping Tiyazo AIAgent."""
    ...
    _ADVERTISED_COMMANDS = (
        ...
        {
            "name": "version",
            "description": "Show Tiyazo version",
        },
    )
    ...
    def _session_modes(self, state: SessionState) -> SessionModeState:
        ...
        modes, which coexist with the model picker, so Tiyazo maps edit approval
    ...
    def _build_model_state(self, state: SessionState) -> SessionModelState | None:
        ...
        try:
            from tiyazo_cli.models import curated_models_for_provider, normalize_provider, provider_label
    ...
    def _resolve_model_selection(raw_model: str, current_provider: str) -> tuple[str, str]:
        ...
        try:
            from tiyazo_cli.models import detect_provider_for_model, parse_model_input
    ...
    def _build_usage_update(state: SessionState) -> UsageUpdate | None:
        ...
        the current request pressure.  Tiyazo estimates ``used`` from the same
    ...
    def _provenance_meta(
        self,
        acp_session_id: str,
        current_tiyazo_session_id: str,
        previous_tiyazo_session_id: Optional[str] = None,
    ) -> Optional[dict]:
        """Best-effort ``_meta.tiyazo.sessionProvenance`` for an ACP session."""
        try:
            return session_provenance_meta(
                self.session_manager._get_db(),
                acp_session_id,
                current_tiyazo_session_id,
                previous_tiyazo_session_id=previous_tiyazo_session_id,
            )
        except Exception:
            logger.debug(
    ...
    async def _send_session_info_update(
        ...
        session_id: str,
        *,
        current_tiyazo_session_id: Optional[str] = None,
        previous_tiyazo_session_id: Optional[str] = None,
    ) -> None:
        """Send ACP native session metadata after Tiyazo changes it.

        When the internal Tiyazo head rotated (e.g. compression-driven session
        split during a turn), pass ``previous_tiyazo_session_id`` so the
        attached ``_meta.tiyazo.sessionProvenance`` flags the rotation reason.
        """
        if not self._conn:
            return
        try:
        ...
        # tiyazo_state.py schema — only started_at/ended_at). Use "now" as
        ...
        meta = self._provenance_meta(
            session_id,
            current_tiyazo_session_id or session_id,
            previous_tiyazo_session_id,
        )
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            enabled_toolsets = _expand_acp_enabled_toolsets(
                getattr(state.agent, "enabled_toolsets", None) or ["tiyazo-acp"],
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="tiyazo-agent", version=TIYAZO_VERSION),
    ...
    async def authenticate(self, method_id: str, **kwargs: Any) -> AuthenticateResponse | None:
        ...
        # Tiyazo' threat model (ACP is stdio-only, local-trust), but poor
        ...
        if normalized_method == TERMINAL_SETUP_AUTH_METHOD_ID:
            # Terminal auth launches Tiyazo setup/model selection out-of-band.
    ...
    async def _replay_session_history(self, state: SessionState) -> None:
        ...
        notifications. Merely restoring server-side state makes Tiyazo
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run Tiyazo on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
        ...
        # ACP thought panes should not receive Tiyazo' local kawaii waiting/status
        ...
        # tools.approval (set_tiyazo_interactive_context) rather than
        # os.environ["TIYAZO_INTERACTIVE"], so concurrent executor workers can't
        ...
        # callback shape — not the gateway-queue TIYAZO_EXEC_ASK path,
        ...
        def _run_agent() -> dict:
            ...
            # Bind TIYAZO_SESSION_KEY for this session so per-session caches
            ...
            interactive_token = set_tiyazo_interactive_context(True)
            ...
            previous_session_id = os.environ.get("TIYAZO_SESSION_ID")
            os.environ["TIYAZO_SESSION_ID"] = session_id
            try:
                ...
                if interactive_token is not None:
                    reset_tiyazo_interactive_context(interactive_token)
                # Restore TIYAZO_SESSION_ID symmetrically.
                if previous_session_id is None:
                    os.environ.pop("TIYAZO_SESSION_ID", None)
                else:
                    os.environ["TIYAZO_SESSION_ID"] = previous_session_id
                if approval_cb:
                    try:
                        from tools import terminal_tool as _terminal_tool
        ...
        try:
            # Snapshot the internal Tiyazo DB session id before the turn so we
            ...
            pre_turn_tiyazo_id = getattr(state.agent, "session_id", None)
            # Wrap the executor call in a fresh copy of the current context so
            # concurrent ACP sessions on the shared ThreadPoolExecutor don't
            # stomp on each other's ContextVar writes (TIYAZO_SESSION_KEY in
        ...
        # _meta.tiyazo.sessionProvenance so ACP clients can render the boundary
        # and keep old/new ids in lineage. The ACP session_id is unchanged.
        post_turn_tiyazo_id = getattr(state.agent, "session_id", None)
        if (
            conn
            and post_turn_tiyazo_id
            and pre_turn_tiyazo_id
            and post_turn_tiyazo_id != pre_turn_tiyazo_id
        ):
            try:
                await self._send_session_info_update(
                    session_id,
                    current_tiyazo_session_id=post_turn_tiyazo_id,
                    previous_tiyazo_session_id=pre_turn_tiyazo_id,
                )
            except Exception:
                logger.debug(
        ...
        # Tiyazo' local "waiting for model response" interrupt status is metadata,
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"Tiyazo Agent v{TIYAZO_VERSION}"
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when Tiyazo has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
