# ============================================================================
# Tiyazo Agent Installer for Windows
# ============================================================================
# Installation script for Windows (PowerShell).
# Uses uv for fast Python provisioning and package management.
#
# Usage:
#   iex (irm https://tiyazo-agent.nousresearch.com/install.ps1)
#
...
param(
    ...
    [string]$Commit = "",
    [string]$Tag = "",
    [string]$TiyazoHome = $(if ($env:TIYAZO_HOME) { $env:TIYAZO_HOME } else { "$env:LOCALAPPDATA\tiyazo" }),
    [string]$InstallDir = $(if ($env:TIYAZO_HOME) { "$env:TIYAZO_HOME\tiyazo-agent" } else { "$env:LOCALAPPDATA\tiyazo\tiyazo-agent" }),
    ...
    # builds apps/desktop into a launchable Tiyazo.exe.
    #
    # Why opt-in:
    #   * Tiyazo-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Tiyazo.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Tiyazo.exe
    ...
    #     `tiyazo desktop` already builds on demand.
...
$RepoUrlSsh = "git@github.com:aitoolpulse/core-backend-engine.git"
$RepoUrlHttps = "https://github.com/aitoolpulse/core-backend-engine.git"
$PythonVersion = "3.11"
...
function Write-Banner {
    ...
    Write-Host "|             * Tiyazo Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $TiyazoHome)) {
        New-Item -ItemType Directory -Force -Path $TiyazoHome | Out-Null
    }
    $envFile = Join-Path $TiyazoHome ".env"
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $TiyazoHome "node"
...
function Install-Uv {
    # Tiyazo owns its own uv at $TiyazoHome\bin\uv.exe.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (tiyazo_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `tiyazo update` stay in sync.
    $managedUv = Join-Path $TiyazoHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $TiyazoHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $TiyazoHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $TiyazoHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $TiyazoHome "bin"
...
function Resolve-UvCmd {
    ...
    # sibling process and TIYAZO_HOME wasn't propagated).
...
function Resolve-AvailablePythonVersion {
    ...
    # ``$script:PythonVersion = $fallbackVer`` mutation.  Under Tiyazo-Setup.exe
...
function Install-Git {
    ...
    which Tiyazo uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\tiyazo\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Tiyazo needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Tiyazo copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\tiyazo\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``TIYAZO_GIT_BASH_PATH`` (User scope) so Tiyazo can find it in a fresh
    ...
    # Download PortableGit into $TiyazoHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $TiyazoHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Tiyazo features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Tiyazo needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``TIYAZO_GIT_BASH_PATH`` (User env scope) so Tiyazo can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $TiyazoHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $TiyazoHome\git\usr\bin\bash.exe
    $candidates += "$TiyazoHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$TiyazoHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("TIYAZO_GIT_BASH_PATH", $candidate, "User")
            $env:TIYAZO_GIT_BASH_PATH = $candidate
            Write-Info "Set TIYAZO_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Tiyazo may not find Git Bash."
    Write-Info "If needed, set TIYAZO_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    $managedNode = "$TiyazoHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$TiyazoHome\node;$env:Path"
        Write-Success "Node.js $version found (Tiyazo-managed)"
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing Tiyazo-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $TiyazoHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $TiyazoHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\tiyazo-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$TiyazoHome\node") { Remove-Item -Recurse -Force "$TiyazoHome\node" }
                Move-Item $extractedDir.FullName "$TiyazoHome\node"
                ...
                $version = & "$TiyazoHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $TiyazoHome\node\ (portable, user-scoped)"
                $script:HasNode = $true
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\tiyazo-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `tiyazo
                    # update` path (#4735).
                    ...
                    $stashName = "tiyazo-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($Commit) {
                    ...
                    # ff-only pull cannot succeed — mirror ``tiyazo update`` and
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if ($LASTEXITCODE -eq 0) {
                            ...
                            Write-Warn "Review git diff / git status if Tiyazo behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running tiyazo processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/aitoolpulse/core-backend-engine/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/aitoolpulse/core-backend-engine/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/aitoolpulse/core-backend-engine/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\tiyazo-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\tiyazo-agent-extract"
    ...
    # next `tiyazo update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under Tiyazo-Setup.exe
    ...
    if (Test-Path "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running tiyazo process.
        ...
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running tiyazo processes before recreating venv..."
            ...
            try {
                schtasks /Query /FO CSV 2>$null | ConvertFrom-Csv | Where-Object { $_.TaskName -like '*Tiyazo_Gateway*' } | ForEach-Object {
                    $tn = $_.TaskName
            ...
            # The launcher CLI (tiyazo.exe) plus its child tree.
            & taskkill /F /T /IM tiyazo.exe /FI "PID ne $myPid" 2>$null | Out-Null
            # taskkill /IM tiyazo.exe is NOT enough: the gateway/agent that a
            # scheduled task or watchdog autostarts runs as
            # `pythonw.exe -m tiyazo_cli.main gateway run` straight out of
            # venv\Scripts\, so its image name is python/pythonw, not tiyazo.exe.
            ...
            # user (see tiyazo_cli/gateway_windows.py), so the installer always
    ...
    if (Test-Path $venvPythonExe) {
        ...
        # the task fires normally on next logon and `tiyazo update` / the
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `tiyazo.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install tiyazo-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and tiyazo.exe broken with
    ...
    # here, not 30 seconds later when the user runs `tiyazo`.
    if (-not $NoVenv) {
    ...
    if (-not $NoVenv) {
        # uv on Windows can register tiyazo.exe in dist-info/RECORD but fail to
        ...
        # `tiyazo` command while tiyazo-agent.exe / tiyazo-acp.exe exist
...
"@ 2>$null
            if ($LASTEXITCODE -eq 0 -and $scriptNames) {
                ...
                if ($missing.Count -gt 0) {
                    ...
                    if ($stillMissing.Count -gt 0) {
                        ...
                        Write-Info "Workaround: `"$pythonExe`" -m tiyazo_cli.main <command>"
                    } else {
    ...
    # users hit and lazy-import errors from `tiyazo dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        try {
            & $pythonExe -m py_compile "$InstallDir\tiyazo_cli\web_server.py" 2>&1 | Out-Null
            if ($LASTEXITCODE -eq 0) { $webServerSyntaxOk = $true }
        } catch { }
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `tiyazo dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `tiyazo dashboard` should now work."
            } else {
        ...
        if (-not $webServerSyntaxOk) {
            throw "dashboard backend source failed syntax check: tiyazo_cli/web_server.py"
        }
    }
...
function Set-PathVariable {
    Write-Info "Setting up tiyazo command..."
    
    if ($NoVenv) {
        $tiyazoBin = "$InstallDir"
    } else {
        $tiyazoBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so tiyazo is globally available
    # On Windows, the tiyazo.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$tiyazoBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$tiyazoBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $tiyazoBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set TIYAZO_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\tiyazo instead
    # of the Unix default ~/.tiyazo
    $currentTiyazoHome = [Environment]::GetEnvironmentVariable("TIYAZO_HOME", "User")
    if (-not $currentTiyazoHome -or $currentTiyazoHome -ne $TiyazoHome) {
        [Environment]::SetEnvironmentVariable("TIYAZO_HOME", $TiyazoHome, "User")
        Write-Success "Set TIYAZO_HOME=$TiyazoHome"
    }
    $env:TIYAZO_HOME = $TiyazoHome
    
    # Update current session
    $env:Path = "$tiyazoBin;$env:Path"
    
    Write-Success "tiyazo command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.tiyazo-bootstrap-complete which tells the Tiyazo
    ...
    # Tiyazo-Setup.exe) or fall back to whatever git resolves in the
    # checkout. The desktop validates schemaVersion + pinnedCommit
    # length but doesn't enforce that HEAD matches the pin (users
    # update via `tiyazo update` which moves HEAD legitimately).
    ...
    $markerPath = Join-Path $InstallDir ".tiyazo-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the TIYAZO_HOME directory structure ($TiyazoHome, default %LOCALAPPDATA%\tiyazo)
    New-Item -ItemType Directory -Force -Path "$TiyazoHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$TiyazoHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$TiyazoHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$TiyazoHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$TiyazoHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$TiyazoHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$TiyazoHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$TiyazoHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$TiyazoHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$TiyazoHome\.env"
    ...
    # Create config.yaml
    $configPath = "$TiyazoHome\config.yaml"
    ...
    # (the default PS5 behaviour), and Tiyazo's prompt-injection scanner
    ...
    # PowerShell version.
    $soulPath = "$TiyazoHome\SOUL.md"
    if (-not (Test-Path $soulPath)) {
        # MUST match DEFAULT_SOUL_MD in tiyazo_cli/default_soul.py. The runtime
...
"@
    ...
    Write-Success "Configuration directory ready: $TiyazoHome"
    
    # Seed bundled skills into $TiyazoHome\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $TiyazoHome\skills ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to $TiyazoHome\skills"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$TiyazoHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $TiyazoHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Tiyazo-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'tiyazo setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\tiyazo-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\tiyazo-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\tiyazo-npm-tui-$(Get-Random).log"
...
# the final `electron` -> `Tiyazo` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable Tiyazo.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_tiyazo_desktop command
    # resolves apps/desktop/release/win-unpacked/Tiyazo.exe directly,
    ...
    # enforces the build floor (^20.19 || >=22.12) and prepends the Tiyazo-managed
    ...
    # The --dir mode produces an unpacked Tiyazo.exe in
    ...
    # are NOT trying to work around). The Tiyazo icon + product name are
    # stamped onto Tiyazo.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\tiyazo-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Tiyazo.exe",
        "$desktopDir\release\win-arm64-unpacked\Tiyazo.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Tiyazo.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Tiyazo icon + identity are stamped onto Tiyazo.exe by the
    ...
    #    Tiyazo.exe. We deliberately do NOT point them at `tiyazo desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Tiyazo.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Tiyazo.lnk')
        )
        ...
        # with the Tiyazo icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.tiyazo/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'tiyazo setup'."
        return
    }
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m tiyazo_cli.main setup
    } else {
        python -m tiyazo_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $tiyazoCmd = "$InstallDir\venv\Scripts\tiyazo.exe"
    if (-not (Test-Path $tiyazoCmd)) {
        $tiyazoCmd = "tiyazo"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$TiyazoHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'tiyazo whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: tiyazo gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$TiyazoHome\logs\gateway.log"
            Start-Process -FilePath $tiyazoCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$TiyazoHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: tiyazo gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: tiyazo gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$TiyazoHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$TiyazoHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$TiyazoHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$TiyazoHome\tiyazo-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   tiyazo              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   tiyazo setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   tiyazo config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   tiyazo config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   tiyazo gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   tiyazo update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Tiyazo repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
if ($IncludeDesktop) {
    ...
    # (Tiyazo-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Tiyazo to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://tiyazo-agent.nousresearch.com/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
