...
Usage (via cron with --no-agent):
    ...
    tiyazo cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$TIYAZO_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Tiyazo-Watcher/1.0"})
