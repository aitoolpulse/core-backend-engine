...
upstream prefix cache warm.  See ``tiyazo-agent-dev``'s
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Tiyazo never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want TIYAZO_HOME persona while keeping
    ...
    # Pointer to the tiyazo-agent skill + docs for user questions about Tiyazo itself.
    stable_parts.append(TIYAZO_AGENT_HELP_GUIDANCE)
    ...
    # TIYAZO_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Coding posture (base Tiyazo, any interactive coding surface in a code
    ...
    # Active-profile hint — names the Tiyazo profile the agent is running
    # under so it doesn't conflate ~/.tiyazo/skills/ (default profile) with
    # ~/.tiyazo/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            "Active Tiyazo profile: default. Other profiles (if any) live "
            "under ~/.tiyazo/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            f"Active Tiyazo profile: {active_profile}. This session reads "
            f"and writes ~/.tiyazo/profiles/{active_profile}/. The default "
            f"profile's data lives at ~/.tiyazo/skills/, ~/.tiyazo/plugins/, "
            f"~/.tiyazo/cron/, ~/.tiyazo/memories/ — those belong to a "
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — Tiyazo never rebuilds or reinjects parts of it
