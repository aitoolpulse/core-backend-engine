#!/usr/bin/env python3
"""Read the in-app terminal pane in the Tiyazo desktop GUI.

This tool is only available when the agent is running under the desktop app.
...
def read_terminal_tool(
    ...
    if callback is None:
        return tool_error("read_terminal is only available in the Tiyazo desktop app.")
...
def check_read_terminal_requirements() -> bool:
    """Desktop GUI only — TIYAZO_DESKTOP is set on the gateway the app spawns."""
    return env_var_enabled("TIYAZO_DESKTOP")


READ_TERMINAL_SCHEMA = {
    "name": "read_terminal",
    "description": (
        "Read what's currently shown in the in-app terminal pane of the Tiyazo "
